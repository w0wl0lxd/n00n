...
use noon_agent::tools::{DescriptionContext, ToolAudience, ToolFilter, ToolRegistry, ToolSource};
use noon_config::{PluginFileConfig, PluginsConfig};
use regex::Regex;
...
use noon_lua::{OptionType, PluginHost};
...
const SECTIONS: &[(&str, &[&str])] = &[
    ...
    (
        "Agent & Knowledge",
        &["almas", "task", "todo_write", "memory", "skill"],
    ),
...
/// `bash` interpolates `noon.uv.cwd()` and `websearch` interpolates
...
fn collect_tool_info(
    ...
    entry: &noon_agent::tools::RegisteredTool,
) -> Option<ToolInfo> {
    let name = entry.name();
...
fn load_registry_with_builtins() -> (Arc<ToolRegistry>, HashSet<String>) {
    ...
    for &sub in noon_config::EDIT_SUB_TOOLS {
...
pub fn generate() -> String {
    ...
    writeln!(
        out,
        "Noon ships with {total} built-in tools. This is the full reference."
    )
    .unwrap();
