use noon_ui::keybindings::{ALT_SEP, KEYBINDS, KeyLabel, KeybindContext, Platform, all_contexts};
...
fn write_overrides(out: &mut String) {
    ...
    out.push_str(
        "Plugins and `init.lua` can rebind keys at runtime with \
         `noon.keymap.set` and `noon.keymap.del`. The tables above are the \
    ...
    out.push_str(
        "1. **Suspend** (`Ctrl+Z`, Unix). Always wins, non-remappable.\n\
         ...
         3. **Lua overrides** from `noon.keymap.set`. Last set wins; \
         binding the same key twice warns.\n\
         4. **Built-in defaults.** An override on the same key shadows \
         them; `noon.keymap.del` lifts the override so the default returns. \
    ...
    out.push_str(
        "If an override leaves Noon stuck (a rebound `Ctrl+C`, a modal \
