use noon_providers::model::{ModelEntry, ModelTier, models_for_provider};
...
const TIER_PICKER_NOTE: &str = r#"Open the model picker with `/model` and press `!`, `@`, `#`, or `$` on any row to assign it to strong, medium, weak, or compaction. Press the same key again to remove the assignment. Your overrides are saved to `~/.local/state/noon/model-tiers` and apply across sessions."#;
...
const OPENCODE_FREE_MODELS_NOTE: &str = r#"By default Noon hides free models from the Opencode catalog. To list free models (they use a public fallback, no API key needed), add this to `~/.config/noon/providers.toml`:
        ...
        match kind {
            ...
            ProviderKind::Copilot => {
                sections.push(ProviderSection {
                    ...
                    name: kind.display_name(),
                    auth_line: format!(
                        "{} (or run `noon auth login copilot` to import a token from gh)",
                        format_auth(kind)
                    ),
                    urls: vec![kind.base_url()],
                    features: kind.features(),
...
pub fn generate() -> String {
    ...
    let _ = writeln!(
        out,
        "Noon talks to LLM providers over their HTTP APIs. \
