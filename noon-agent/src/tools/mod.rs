...
use noon_providers::RequestOptions;
...
/// list a Lua caller holds, e.g. `noon.api.get_tools`).
...
pub struct ToolContext {
    ...
    pub timeouts: noon_providers::Timeouts,
    ...
    /// caller (`noon.agent.call_tool` with `on_live_buf`/`on_annotation`).
...
static HOME: LazyLock<Option<PathBuf>> = LazyLock::new(noon_storage::paths::home);
...
pub fn is_builtin_tool(name: &str) -> bool {
    noon_config::DEFAULT_BUILTINS.contains(&name) || noon_config::EDIT_SUB_TOOLS.contains(&name)
}

pub fn all_builtin_tool_names() -> Vec<&'static str> {
    noon_config::DEFAULT_BUILTINS
        .iter()
        .chain(noon_config::EDIT_SUB_TOOLS.iter())
        .copied()
        .collect()
}

use noon_providers::{Message, ProviderEvent, StreamResponse};
...
impl Provider for NullProvider {
    ...
    fn list_models(
        &self,
    ) -> BoxFuture<'_, Result<Vec<noon_providers::ModelInfo>, crate::AgentError>> {
        Box::pin(async { unimplemented!() })
    }
}
...
pub fn interpreter_ctx(
    ...
    ToolContext {
        ...
        permissions,
        timeouts: noon_providers::Timeouts::default(),
        file_tracker,
...
/// Minimal ToolContext for CLI one-shot tool execution (e.g. `noon index`).
...
pub fn cli_tool_ctx() -> ToolContext {
    ...
    interpreter_ctx(
        ...
        Arc::new(PermissionManager::new(
            noon_config::PermissionsConfig {
                default: noon_config::DefaultEffect::Allow,
                rules: vec![],
                ..Default::default()
            },
