...
use flume::Sender;
use noon_config::ToolKey;
use noon_providers::{AgentError, ContentBlock, Message, Role, StopReason, TokenUsage};
...
pub enum ToolOutput {
    ...
    Image {
        source: noon_providers::ImageSource,
...
impl ToolOutput {
    ...
    /// The UI uses it on tool completion, and `noon.agent.call_tool` falls
...
mod tests {
    ...
    fn tool_results_appends_images_after_all_results() {
        let image = |data: &str| ToolOutput::Image {
            source: noon_providers::ImageSource::new(
                noon_providers::ImageMediaType::Png,
                Arc::from(data),
            ),
