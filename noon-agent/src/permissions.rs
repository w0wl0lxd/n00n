...
fn builtin_rules(cwd: &Path) -> Vec<PermissionRule> {
    let cwd_glob = format!(
        "{}/**",
        noon_storage::paths::canonicalize_clean(cwd).display()
    );
...
pub fn scope_matches(pattern: &str, value: &str) -> bool {
    ...
    if let Some(prefix) = pattern.strip_suffix("/**") {
        let norm_prefix = noon_storage::paths::canonicalize_clean(Path::new(prefix));
        ...
        let norm_value = noon_storage::paths::incremental_canonicalize(Path::new(value))
            .unwrap_or_else(|| noon_storage::paths::normalize_path(Path::new(value)));
...
pub fn normalize_scope_path(path: &str) -> String {
    ...
    noon_storage::paths::normalize_path(Path::new(&resolved))
        .to_string_lossy()
        .into_owned()
}
...
pub fn physical_boundary_check(parent: &Path, child: &Path) -> Option<bool> {
    let parent_canon = noon_storage::paths::incremental_canonicalize(parent)?;
    let child_canon =
        noon_storage::paths::incremental_canonicalize(child).unwrap_or_else(|| child.to_path_buf());
...
mod tests {
    ...
    fn scope_matches_resolves_symlinked_parent() {
        ...
        let real = tmp.join("__noon_test_scope_symlink_real");
        let link = tmp.join("__noon_test_scope_symlink_link");
    ...
    fn boundary_dotdot_smuggling_proceeds_via_prompt() {
        let tmp = std::env::temp_dir();
        let sub = tmp.join("__noon_test_boundary");
    ...
    fn boundary_symlink_escape_proceeds_via_prompt() {
        ...
        let tmp = std::env::temp_dir();
        let project = tmp.join("__noon_test_symlink_escape");
    ...
    fn boundary_nonexistent_cwd_proceeds_via_lexical_tail() {
        let missing = std::env::temp_dir().join("__noon_test_absent_cwd_xyz");
    ...
    fn plan_path_auto_allows_file_write_tools_only(tool: &str, expect_allowed: bool) {
        let plan = "/home/user/.local/state/noon/plans/test.md";
