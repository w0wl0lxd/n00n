...
pub fn resolve_compaction_model(
    ...
    if let Some(spec) = noon_providers::model_registry::model_registry()
        .read()
        .unwrap()
        .spec_for_tier_any(noon_providers::ModelTier::Compaction)
        && let Ok(mut m) = Model::from_spec(&spec)
        && let Ok(p) = noon_providers::provider::from_model(&mut m, timeouts)
    {
        return (Arc::from(p), m);
    }
...
mod tests {
    ...
    use noon_providers::provider::{BoxFuture, Provider};
    use noon_providers::{
    ...
    impl Provider for MockProvider {
        ...
        fn list_models(&self) -> BoxFuture<'_, Result<Vec<noon_providers::ModelInfo>, AgentError>> {
    ...
    fn cancel_token_aborts_during_api_call() {
        smol::block_on(async {
            ...
            impl Provider for HangingProvider {
                ...
                fn list_models(
                    &self,
                ) -> BoxFuture<'_, Result<Vec<noon_providers::ModelInfo>, AgentError>>
                {
                    Box::pin(async { unimplemented!() })
                }
            }
