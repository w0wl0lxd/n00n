...
use noon_config::CompactionBuffer;
use noon_providers::{
...
pub(super) async fn compact_history(
    provider: &dyn noon_providers::provider::Provider,
    model: &Model,
    history: &mut History,
...
pub(super) fn auto_compact_enabled() -> bool {
    env::var("NOON_DISABLE_AUTOCOMPACT")
        .map(|v| v != "1" && v != "true")
        .unwrap_or(true)
}
...
mod tests {
    ...
    fn strip_images_replaces_with_placeholder() {
        use noon_providers::{ImageMediaType, ImageSource};
        use std::sync::Arc;
