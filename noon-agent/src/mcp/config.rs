...
use noon_config::{global_config_dir, is_valid_server_name};
...
pub(super) use noon_config::is_valid_wire_name as is_valid_tool_name;
...
pub fn load_config(cwd: &Path) -> (McpConfig, McpConfigErrors) {
    ...
    let project_path = cwd.join(".noon").join(MCP_CONFIG_FILE);
...
"#,
        ...
        let project_noon_dir = project_dir.join(".noon");
        fs::create_dir_all(&project_noon_dir).unwrap();
        fs::write(
            project_noon_dir.join("mcp.toml"),
            r#"[mcp.srv]
command = ["project"]
"#,
        )
        ...
        let project_cfg = read_config(&project_noon_dir.join("mcp.toml"))
            .unwrap()
            .unwrap();
