...
pub const DEFAULT_IDENTITY: &str = r#"You are Noon, an interactive CLI coding agent. Use the tools available to assist the user with software engineering tasks. Complete tasks successfully while minimizing token usage and tool calls to avoid context bloat.
...
mod tests {
    ...
    fn empty_slots_emit_template_and_native_efficient_line() {
        ...
        assert!(out.starts_with("You are Noon"));
        assert!(
            !out.contains("{{"),
    ...
    fn singleton_entry_replaces_default() {
        ...
        assert!(!out.contains("You are Noon"));
    }
