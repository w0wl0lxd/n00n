...
use noon_agent::types::AgentEvent;
use noon_agent::{AgentInput, AgentMode, Envelope, ImageMediaType, ImageSource};
use noon_providers::Message;
use noon_providers::model::Model;
use noon_providers::provider::available_model_specs;
use noon_storage::id::{NoonId, SessionRef};
use serde::Serialize;
...
fn load_history(session_id: NoonId) -> Result<Vec<Message>, AcpError> {
    let storage = noon_storage::StateDir::resolve()
        .map_err(|e| AcpError::internal_error().data(json_str(&e)))?;
    load_history_from(&storage, session_id)
}

fn load_history_from(
    storage: &noon_storage::StateDir,
    session_id: NoonId,
) -> Result<Vec<Message>, AcpError> {
    let session: noon_storage::sessions::Session<
        Message,
        noon_providers::TokenUsage,
        noon_agent::ToolOutput,
    > = noon_storage::sessions::Session::load(session_id, storage).map_err(|e| {
...
#[cfg(test)]
mod tests {
    use noon_providers::{ContentBlock as MsgBlock, Role, TokenUsage};
    ...
    fn load_history_round_trips_stored_messages() {
        ...
        let mut session: Session<Message, TokenUsage, noon_agent::ToolOutput> =
            Session::new("anthropic/test-model", "/project");
        session.messages = messages.clone();
        session.save(&dir).unwrap();

        let id: NoonId = session.id;
    ...
    fn load_missing_session_is_resource_not_found() {
        let tmp = TempDir::new().unwrap();
        let dir = StateDir::from_path(tmp.path().to_path_buf());
        let err = load_history_from(&dir, NoonId::generate()).unwrap_err();
