...
pub fn initialize_response() -> InitializeResponse {
    InitializeResponse::new(ProtocolVersion::V1)
        ...
        .auth_methods(vec![])
        .agent_info(Implementation::new("noon", VERSION))
}
...
pub fn mode_id_to_agent_mode(mode_id: &str) -> Option<noon_agent::AgentMode> {
    match mode_id {
        MODE_BUILD => Some(noon_agent::AgentMode::Build),
        MODE_PLAN => {
            let storage = noon_storage::StateDir::resolve().ok()?;
            let plan_path = noon_storage::plans::new_plan_path(&storage).ok()?;
            Some(noon_agent::AgentMode::Plan(plan_path))
        }
        _ => None,
    }
}
