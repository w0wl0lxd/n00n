...
impl<T: Transport> Extractor<T> {
    ...
    pub fn new(transport: T, policy: UrlPolicy, limits: FetchLimits) -> Result<Self, Error> {
        Ok(Self {
    ...
    pub async fn extract(&self, request: &ExtractRequest) -> Result<ExtractResponse, Error> {
        request.validate()?;
        ...
        if results.is_empty() {
            return Err(first_error.ok_or_else(|| {
                Error::validation("urls", "operation produced no usable output")
            })?);
        }
...
fn render_content(
    ...
    let content = match format {
        ...
        ExtractFormat::Text if is_html(content_type) => {
            let markdown = htmd::convert(decoded.as_ref()).map_err(|error| Error::Parse {
                message: error.to_string(),
            })?;
...
fn error_kind(error: &Error) -> &'static str {
    match error {
        ...
        Error::PolicyDenied { .. } => "policy_denial",
        Error::Auth { .. } => "auth",
