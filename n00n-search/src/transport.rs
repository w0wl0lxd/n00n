...
async fn resolve_and_validate(url: &ValidatedUrl) -> Result<Option<ResolveMap>, Error> {
    ...
    let port = url
        ...
        .ok_or_else(|| Error::validation("url", "port is required"))?;
...
impl<T: Transport> Fetcher<T> {
    ...
    pub async fn fetch(&self, input: &str) -> Result<FetchResponse, Error> {
...
mod tests {
    ...
    use crate::{error::Error, url_policy::UrlPolicy};
