...
Allowed: read-only built-ins, web/search, CodeGraph, Arbor, and MCP tools explicitly marked read-only. Missing or stale project indexes refresh automatically. `run_shell` accepts only read-only inspection commands. `run_python` is unavailable. Use `write_file`, `edit_file` and `edit_file_bulk` only for `{plan_path}`.
