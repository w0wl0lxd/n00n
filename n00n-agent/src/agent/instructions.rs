...
const BUILD_PLAN_HEADER: &str = "<system-reminder>\n# Approved Plan\n\nThe user has approved a plan. Follow it to implement the request.\n\n";
...
fn collect_instruction_files(
    ...
    for path in n00n_storage::paths::user_config_dirs(home, xdg_config, "AGENTS.md") {
...
pub fn load_instruction_text(cwd: &str) -> String {
    load_instruction_text_with_home(
        cwd,
        n00n_storage::paths::home().as_deref(),
        n00n_storage::paths::config_dir().ok().as_deref(),
    )
}
...
mod tests {
    ...
    const PLAN_PATH: &str = ".n00n/plans/123.md";
