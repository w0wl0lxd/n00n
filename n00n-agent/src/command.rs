...
const PROJECT_COMMAND_DIRS: &[&str] = &[".n00n/commands", ".claude/commands"];
...
mod tests {
    ...
    fn discover_project_overrides_global() {
        ...
        let cmd_dir = project.path().join(".n00n/commands");
        ...
        let global_cmd_dir = global.path().join(".n00n/commands");
    ...
    fn discover_ignores_non_md_files() {
        ...
        let cmd_dir = dir.path().join(".n00n/commands");
        fs::create_dir_all(&cmd_dir).unwrap();
