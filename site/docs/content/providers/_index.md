...
The `base` field specifies which built-in provider to inherit the model catalog from. Valid values: `anthropic`, `openai`, `codex`, `google`, `copilot`, `ollama`, `llama-cpp`, `mistral`, `zai`, `deepseek`, `openrouter`, `synthetic`, `tensorx`, `opencode`, `devin`.
