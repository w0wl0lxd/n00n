...
pub struct RestoreItem {
    ...
    pub input: Value,
    pub is_error: bool,
    pub tool_output_lines: noon_config::ToolOutputLines,
...
pub(crate) struct ClickFallback {
    ...
    pub event_tx: noon_agent::EventSender,
}
...
impl RestoreReply {
    pub(crate) fn emit(
        ...
        if let Some(snapshot) = self.body {
            let _ = event_tx.send(noon_agent::AgentEvent::ToolSnapshot {
        ...
        if let Some(snapshot) = self.header {
            let _ = event_tx.send(noon_agent::AgentEvent::ToolHeaderSnapshot {
                id: tool_use_id.to_owned(),
                snapshot,
                theme_gen,
            });
...
pub(crate) struct TaskCell {
    ...
    /// `noon.agent.call_tool(on_live_buf/on_annotation)`.
    pub(crate) live_sink: Option<flume::Sender<ToolLive>>,
    /// When `Some`, `noon.async.run` tasks queue here instead of the global
...
/// queued `noon.async.run` task holds one, so the `Arc` strong count is the
...
/// Channel of `noon.async.run` tasks. The dispatcher recvs the `rx` side as
...
fn spawn_async_task(
    ...
    ex.spawn(async move {
            ...
            && let Some(buf) = task.owner.as_ref().and_then(|c| c.root_buf())
        {
            ...
            let _ = live.event_tx.send(noon_agent::AgentEvent::ToolSnapshot {
                id: live.tool_use_id.clone(),
                snapshot: noon_agent::BufferSnapshot::from_arc(buf.read()),
                theme_gen: None,
            });
        }
...
/// Barrier for load/clear ops: drains queued `noon.async.run` tasks and
...
impl LuaRuntime {
    ...
    /// (like `noon.truncate`) without touching the filesystem.
    ...
    /// `noon.api.register_options`; if the plugin never declared any, every
    ...
    fn check_opts_consumed(&self, name: &str, opts: &PluginOpts) -> Result<(), mlua::Error> {
        ...
        Err(mlua::Error::runtime(format!(
            "unknown options in plugins.{name}: {} (this plugin declares no options via noon.api.register_options)",
            keys.join(", ")
        )))
    }
    ...
    async fn load_source(
        ...
        if let Some(cs) = config_store {
            let setup_fn = crate::api::util::setup::create_setup_fn(&self.lua, Arc::clone(cs))
                .map_err(&map_err)?;
            noon.set("setup", setup_fn).map_err(&map_err)?;
        }

        let env = self.build_env(noon, require_root).map_err(&map_err)?;
...
async fn restore_item(lua: &Lua, plugins: &PluginMap, item: RestoreItem) -> Option<RestoreReply> {
    ...
    let cell = TaskCell::new(
        ...
        Some(LiveCtx {
            event_tx: noon_agent::EventSender::new(dummy_tx, 0),
...
/// Runs `noon.async.run` tasks queued during restore inline, so their
            ...
            Ok(LuaValue::Nil) => {
                ...
                if let Some(buf) = resolve_root_buf(&handle) {
                    if let Some(live) = live {
                        let _ = live.event_tx.send(noon_agent::AgentEvent::LiveToolBuf {
                            id: live.tool_use_id.clone(),
                            body: Arc::clone(&buf),
                        });
                    }
        ...
        .spawn(move || {
            ...
            smol::block_on(ex.run(async {
                loop {
                    ...
                    // snappy, and queued `noon.async.run` tasks jump ahead of
                    // plain requests.
...
mod tests {
    ...
    fn with_live_ctx_follows_task_live_field() {
        ...
        let with_live = task_cell(Some(LiveCtx {
            event_tx: noon_agent::EventSender::new(tx, 0),
