//! Renders the Lua API reference and the builtin `noon-plugin-dev` skill
//! straight from `api_docs()` at runtime, so no generated files live in the
//! repo. `noon-docgen` calls [`site_page`] for the website; the plugin
...
const NAME: &str = "noon-plugin-dev";
const DESCRIPTION: &str = "Write or modify noon plugins or init.lua config in Lua: custom tools, slash commands, keymaps, UI. Authoring guide, real example, indexed noon Lua API reference. Load before any noon plugin work.";
const REFERENCE_PLACEHOLDER: &str = "__NOON_REFERENCE_PATH__";

const EXAMPLE: &str = include_str!("../../plugins/glob/init.lua");

const GUIDE: &str = r#"# Writing noon plugins

Noon plugins are plain Lua files (Luau) that run inside noon. A plugin can
register tools the LLM calls, slash commands, keymaps, prompt hints, and
custom UI. Everything lives under the global `noon` table. An index of the
full API reference is at the end of this document.

## Where plugin code goes

- `~/.config/noon/init.lua` - global, loaded for every project
- `<project>/.noon/init.lua` - project-local

Either file can call `noon.setup({ ... })` for configuration and register
tools or commands. There are no separate plugin directories yet.

## Development loop

You cannot run slash commands or restart noon. After editing, ask the user
...
To debug, add `noon.log.info|warn|error(...)` calls. They write to `noon.log`
in the dir `noon.env.logs_dir()` returns (Linux: `~/.local/logs/noon/`).
...
- Reusable helpers ship with noon; see "Shared helper modules" in the reference file.
...
The API mirrors Neovim where possible (`noon.fs`, `noon.uv`, `noon.treesitter`,
`noon.keymap`, `noon.base64`); signatures are identical so code can be
...
/// `noon-plugin-dev` skill from here instead of generated files on disk.
...
fn helpers() -> Vec<(String, &'static str)> {
    let noon = lib_dir()
        .get_dir("noon")
        .expect("plugins/lib/noon embedded");
    let mut helpers: Vec<(String, &'static str)> = noon
        .files()
        .filter_map(|file| {
            let path = file.path();
            let stem = path.file_stem()?.to_str()?;
            (path.extension()? == "lua")
                .then(|| (format!("noon.{stem}"), file.contents_utf8().unwrap()))
        })
        .collect();
    helpers.sort();
    helpers
...
mod tests {
    ...
    fn reference_index_lists_headings_with_line_numbers() {
        ...
        let expected = "\n## noon.api - L3\n\
            - L7 `noon.api.register_tool({spec})` - Register a tool.\n\
            - L11 `noon.api.bare()`\n\n\
            ## Shared helper modules - L15\n\
            - L17 `require(\"noon.color\")` - Terminal colors helper.\n";
