...
impl LuaCtx {
    ...
    /// Dispatch capability: only handler ctxs can call `noon.agent.*`.
...
impl UserData for LuaCtx {
    fn add_methods<M: UserDataMethods<Self>>(methods: &mut M) {
        ...
        methods.add_async_method(
            ...
            |lua, this, dir_path: String| async move {
                ...
                let results = smol::unblock(move || {
                    ...
                    noon_agent::find_subdirectory_instructions(&abs, &cwd, &loaded)
                })
                .await;
        ...
        methods.add_method("is_instruction_file", |_, _, name: String| {
            Ok(noon_agent::is_instruction_file(&name))
        });
...
mod tests {
    ...
    use noon_agent::tools::LocalToolFn;
    use noon_agent::tools::test_support::stub_ctx_with;
