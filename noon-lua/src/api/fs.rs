...
pub(crate) fn expand_tilde(path: &str) -> PathBuf {
    if let Some(rest) = path.strip_prefix("~/") {
        if let Some(home) = noon_storage::paths::home() {
            return home.join(rest);
        }
    } else if path == "~"
        && let Some(home) = noon_storage::paths::home()
    {
        return home;
    }
    PathBuf::from(path)
...
async fn glob(lua: Lua, pattern: Value, opts: Option<Table>) -> LuaResult<(Value, Value)> {
    ...
    let result: Result<Vec<String>, String> = smol::unblock(move || {
        let root = noon_agent::tools::resolve_search_path(path.as_deref())?;
        let pattern_refs: Vec<&str> = patterns.iter().map(|s| s.as_str()).collect();

        let walker = noon_agent::tools::walk_builder_opts(&root, &pattern_refs, gitignore)?.build();
        ...
        let paths: Vec<String> = if sort_mtime {
            let mut entries: Vec<_> = iter
                .filter_map(|e| {
                    let p = e.into_path();
                    let mt = noon_agent::tools::mtime(&p);
...
async fn grep(lua: Lua, pattern: String, opts: Option<Table>) -> LuaResult<(Value, Value)> {
    let mut params = noon_agent::tools::grep::GrepParams::new(pattern);
    ...
    let result = smol::unblock(move || noon_agent::tools::grep::grep_search(params)).await;
...
lua_table! {
    ...
    /// if err then return end
    /// ```
    "noon.fs" => pub(crate) fn create_fs_table(perms: &PluginPermissions), DOCS [
