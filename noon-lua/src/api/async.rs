...
///
/// @return (noon.async.Permit) A permit handle. Call `:release()` when done, or let it be garbage collected.
/// @example
...
lua_class! {
    ...
    /// is cancelled too.
    "noon.async.Semaphore" => LuaSemaphore, SEMAPHORE_DOCS [acquire]
}
...
lua_class! {
    ...
    "noon.async.Permit" => LuaPermit, PERMIT_DOCS [release]
}
...
/// @return (noon.async.Semaphore) A new semaphore.
/// @example
...
const await__doc: FnDoc = FnDoc {
    ...
    example: "local result = noon.async.await(2, http.get, url)",
};
...
const wrap__doc: FnDoc = FnDoc {
    name: "wrap",
    args: "{argc}, {fn}",
    desc: "Create a coroutine-friendly wrapper around a callback-based function. The wrapper calls `noon.async.await` for you, so you can use the result like a normal function call.",
    params: &[
        ParamDoc {
            name: "{argc}",
            ty: "integer",
            desc: "Callback position, forwarded to `noon.async.await`.",
        },
        ParamDoc {
            name: "{fn}",
            ty: "function",
    ...
    example: "local get = noon.async.wrap(2, http.get)\nlocal body = get(url)",
};
...
lua_table! {
    ...
    /// })
    /// ```
    extend "noon.async" => pub(crate) fn add_async_fns(), DOCS [
        ...
        lua.create_async_function(|lua, args: MultiValue| async move {
            ...
            if args_vec.len() < AWAIT_MIN_ARGS {
                return Err(mlua::Error::runtime(
                    "noon.async.await requires at least 2 arguments: argc, fun, ...",
                ));
            }
...
mod tests {
    ...
    const ERR_TOO_FEW_ARGS: &str = "noon.async.await requires at least 2 arguments: argc, fun, ...";
