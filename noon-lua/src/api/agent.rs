...
fn resolve_model_from_ctx(ctx: &AgentContext, tier: Option<&str>) -> Result<Model, String> {
    ...
    let map = noon_providers::model_registry::model_registry()
        .read()
        .unwrap();
    ctx.model
        .dynamic_slug
...
fn dispatch_ctx<'a>(ctx: &'a LuaCtx, method: &str) -> Result<&'a AgentContext, String> {
    ctx.agent()
        .ok_or_else(|| ctx.cap_err(&format!("noon.agent.{method}")))
}
...
/// `noon.agent.*` convention: wrong argument types throw; every value or
...
async fn system_prompt(
    ...
    let prompt_id = match prompt_id_str.as_str() {
        "research" => noon_agent::prompt::PromptId::Research,
        "general" => noon_agent::prompt::PromptId::General,
        "system" => noon_agent::prompt::PromptId::System,
        other => return Ok(err_pair(format!("unknown prompt_id: {other}"))),
    };

    let vars = noon_agent::template::env_vars();
    ...
    let instructions = match instructions_val {
        LuaValue::Boolean(true) => {
            ...
            smol::unblock(move || noon_agent::agent::load_instruction_text(&cwd)).await
        }
    ...
    let assembled = noon_agent::prompt::assemble(prompt_id, &agent.prompt_slots, &instructions);
...
async fn tools(lua: Lua, ctx: mlua::UserDataRef<LuaCtx>, opts: Table) -> LuaResult<Pair<LuaValue>> {
    ...
    let filter = base
        .excluding(&disabled)
        .excluding(noon_agent::tools::capability_exclusions(model));
...
async fn session(
    ...
    let state = SessionState {
        params: AgentParams {
            ...
            registry: Arc::clone(noon_agent::tools::ToolRegistry::global_arc()),
            audience,
        },
...
struct SessionState {
    ...
    fast: bool,
    mcp: Option<noon_agent::mcp::McpHandle>,
    history: History,
