...
const iter_captures__doc: FnDoc = FnDoc {
    ...
    example: "local q = noon.treesitter.query.parse(\"lua\", \"(identifier) @id\")\nfor idx, node, meta in q:iter_captures(root, source) do\n  print(idx, node:type())\nend",
};
...
lua_class! {
    ...
    /// end
    /// ```
    "noon.treesitter.Query" => LuaQuery, QUERY_DOCS [manual iter_captures, manual iter_matches] fields query_fields, extra query_methods
}
...
lua_table! {
    ...
    /// ```
    "noon.treesitter.query" => pub(crate) fn create_query_module(), DOCS [
        parse, get,
    ]
}
