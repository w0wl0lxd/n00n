//! Manual performance harness (run with `cargo bench -p noon-lua --bench perf`).
//!
//! Covers the two data-path claims introduced by the axon->noon port:
//!   * PR-C: TOON encoding shrinks structured tool output vs JSON.
//!   * PR-B: `TokenUsage::cost` prices completions from provider pricing.

use noon_providers::model::{Model, ModelPricing, TokenUsage};
use serde_json::json;
