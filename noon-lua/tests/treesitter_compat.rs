...
local parser = noon.treesitter.get_parser(source, "rust")
...
local query = noon.treesitter.query.parse("rust", [[
...
local query = noon.treesitter.query.parse("rust", "(function_item) @fn")
...
local id_query = noon.treesitter.query.parse("rust", "(identifier) @id")
...
local gnr_sr, gnr_sc, gnr_er, gnr_ec = noon.treesitter.get_node_range(root)
assert(gnr_sr == sr and gnr_sc == sc)

local range6 = noon.treesitter.get_range(root)
assert(range6[1] == sr and range6[3] == sb)

assert(noon.treesitter.is_in_node_range(root, 1, 0) == true)

assert(noon.treesitter.node_contains(root, {{1, 0, 2, 0}}) == true)

local child = root:named_child(0)
assert(child ~= nil)
assert(noon.treesitter.is_ancestor(root, child) == true)
assert(noon.treesitter.is_ancestor(child, root) == false)

local text = noon.treesitter.get_node_text(child, source)
...
fn language_module() {
    ...
    run_lua(
        ...
        "language_mod",
        r#"
noon.treesitter.language.add("rust")

noon.treesitter.language.register("rust", "rs")
assert(noon.treesitter.language.get_lang("rs") == "rust")

noon.treesitter.language.register("python", {"py", "pyi"})
assert(noon.treesitter.language.get_lang("py") == "python")
assert(noon.treesitter.language.get_lang("pyi") == "python")

local fts = noon.treesitter.language.get_filetypes("python")
assert(#fts == 2, "expected 2 filetypes for python")

assert(noon.treesitter.language.get_lang("unknown_ext_xyz") == nil)

local info = noon.treesitter.language.inspect("rust")
...
local parser = noon.treesitter.get_parser(source, "python")
local root = parser:parse()[1]:root()
assert(root:type() == "module")

local query = noon.treesitter.query.parse("python", [[
...
local parser = noon.treesitter.get_parser(source, "typescript")
local root = parser:parse()[1]:root()
assert(root:type() == "program")

local query = noon.treesitter.query.parse("typescript", [[
