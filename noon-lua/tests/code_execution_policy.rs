...
use noon_agent::AgentMode;
use noon_agent::tools::test_support::stub_ctx;
use noon_agent::tools::{DescriptionContext, ToolAudience, ToolContext, ToolFilter, ToolRegistry};
...
fn event_ctx(reg: &Arc<ToolRegistry>) -> (ToolContext, flume::Receiver<noon_agent::Envelope>) {
    let (tx, rx) = flume::unbounded::<noon_agent::Envelope>();
    let event_tx = noon_agent::EventSender::new(tx, 0);
    let mut ctx = noon_agent::tools::test_support::stub_ctx_with(
        &AgentMode::Build,
        Some(&event_tx),
        Some(SCRIPT_TOOL_ID),
    );
...
fn parse_code(reg: &ToolRegistry, code: &str) -> Box<dyn noon_agent::tools::ToolInvocation> {
    reg.get("code_execution")
        .expect("code_execution registered")
        .tool
        .parse(&serde_json::json!({ "code": code, "timeout": 10 }))
...
fn start_preview_text(code: &str) -> String {
    ...
    let body = rx
        ...
        .find_map(|env| match env.event {
            noon_agent::AgentEvent::LiveToolBuf { id, body } if id == SCRIPT_TOOL_ID => Some(body),
            _ => None,
        })
...
fn final_body_text(rx: &flume::Receiver<noon_agent::Envelope>) -> String {
    rx.drain()
        .filter_map(|env| match env.event {
            noon_agent::AgentEvent::ToolSnapshot { id, snapshot, .. } if id == SCRIPT_TOOL_ID => {
                Some(snapshot.text())
            }
            noon_agent::AgentEvent::LiveToolBuf { id, body } if id == SCRIPT_TOOL_ID => {
                Some(body.take().text())
            }
            _ => None,
        })
...
fn restore_lines_with(code: &str, output: &str, is_error: bool, clicks: Vec<usize>) -> Vec<String> {
    ...
    eh.request_restore(
        noon_lua::RestoreItem {
            ...
            is_error,
            tool_output_lines: noon_config::ToolOutputLines::default(),
            theme_gen: None,
            clicks,
            state: None,
        },
    ...
    let snapshot = loop {
        ...
        if let noon_agent::AgentEvent::ToolSnapshot { id, snapshot, .. } = env.event
            && id == SCRIPT_TOOL_ID
        {
            break snapshot;
        }
