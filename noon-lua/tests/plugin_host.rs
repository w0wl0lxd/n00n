...
use noon_agent::tools::{ToolRegistry, ToolSource, timeout_annotation};
use noon_config::{AlwaysThinking, PluginsConfig, ToolOutputLines};
use noon_lua::{PluginError, PluginHost, WARM_TOOL_CAP};
use std::path::Path;
...
fn exec_tool_in(
    ...
    exec_output_in(reg, name, input, registry_override).map(|out| match out {
        noon_agent::ToolOutput::Plain(s) => s.text,
...
fn exec_tool_output(
    ...
    name: &str,
    input: serde_json::Value,
) -> Result<noon_agent::ToolOutput, String> {
...
fn exec_output_in(
    ...
    let mut ctx = noon_agent::tools::test_support::stub_ctx(&noon_agent::AgentMode::Build);
...
fn get_tool_returns_normalized_header_and_restore_handles() {
    ...
    let src = format!(
        ...
        noon.api.register_tool({{
            ...
            schema = {MINIMAL_SCHEMA},
            handler = function()
                local t = noon.api.get_tool("styled_tool")
                if not t then return nil, "not found" end
                local thrower = noon.api.get_tool("throwing_tool")
                ...
                return table.concat({{
                    ...
                    tostring(noon.api.get_tool("nope_tool") == nil),
                    type(noon.api.get_tool("handle_probe").header),
                }}, "|")
            end
        }})
        "#,
...
fn handler_state_flows_to_tool_output_and_serde() {
    ...
    let parsed: noon_agent::ToolOutput = serde_json::from_str(&json).unwrap();
...
fn restore_snapshot_text(
    ...
    handle.request_restore(
        ...
        noon_agent::EventSender::new(tx, 0),
    );
    handle.wait_restore_complete_for_test();

    let mut text = String::new();
    for env in rx.drain() {
        if let noon_agent::AgentEvent::ToolSnapshot { snapshot, .. } = env.event {
...
noon.api.register_tool({{
    ...
    audiences = {{ "main" }},
    handler = function(input, ctx)
        local t = noon.api.get_tool("child_r")
        local parts = {{}}
    ...
    let src = format!(
        r#"noon.api.register_tool({{
            ...
            handler = function(input, ctx)
                ...
                local parts = {{}}
                parts[1] = pair_err(noon.agent.system_prompt(ctx, {{ prompt_id = "nope" }})) and "prompt_err" or "prompt_ok"
                parts[2] = pair_err(noon.agent.tools(ctx, {{ audience = "nope" }})) and "tools_err" or "tools_ok"
                parts[3] = pair_err(noon.agent.resolve_model(ctx, {{ spec = "not-a-spec" }})) and "model_err" or "model_ok"
                return table.concat(parts, " ")
            end
        }})"#
    );
...
/// Restore used to lose anything drawn via `noon.async.run`: those tasks
...
"#,
    ...
    let inv = entry.tool.parse(&serde_json::json!({})).unwrap();
    let mut ctx = noon_agent::tools::test_support::stub_ctx(&noon_agent::AgentMode::Build);
    ctx.deadline = noon_agent::tools::Deadline::after(std::time::Duration::from_secs(5));
...
fn handler_nil_without_jobs_is_error() {
    let reg = fresh_registry();
    let host = PluginHost::new(Arc::clone(&reg)).unwrap();
    let src = r#"noon.api.register_tool({
        name = "nil_no_jobs",
...
fn jobstart_invalid_cwd_errors_with_expanded_path() {
    ...
    let src = format!(
        r#"noon.api.register_tool({{
            ...
            audiences = {{ "main" }},
            handler = function(input, ctx)
                local _, err = pcall(noon.fn.jobstart, "pwd", {{ cwd = "{JOB_BAD_CWD}" }})
                return tostring(err)
            end
        }})"#,
    );
    ...
    let expanded = noon_storage::paths::home()
        .expect("home dir")
...
fn click_until_finished(
    ...
    let worker = std::thread::spawn(move || {
        let ctx = noon_agent::tools::test_support::stub_ctx_with(
...
fn warm_restore_item(id: &str, clicks: Vec<usize>) -> noon_lua::RestoreItem {
    noon_lua::RestoreItem {
...
fn snapshot_texts(rx: &flume::Receiver<noon_agent::Envelope>, id: &str) -> Vec<String> {
    rx.drain()
        .filter_map(|env| match env.event {
            noon_agent::AgentEvent::ToolSnapshot {
                id: got, snapshot, ..
            } if got == id => Some(
                snapshot
                    .lines
...
fn warm_ctx(
    ...
    noon_agent::tools::ToolContext,
    flume::Receiver<noon_agent::Envelope>,
) {
    let (tx, rx) = flume::unbounded::<noon_agent::Envelope>();
    let event_tx = noon_agent::EventSender::new(tx, 0);
    let ctx = noon_agent::tools::test_support::stub_ctx_with(
        &noon_agent::AgentMode::Build,
        Some(&event_tx),
        Some(id),
...
fn exec_warm_tool(
    reg: &ToolRegistry,
    tool: &str,
    ctx: &noon_agent::tools::ToolContext,
) -> Result<noon_agent::ToolOutput, String> {
    let inv = reg
        .get(tool)
...
fn warm_click_reaches_finished_tool(is_error: bool) {
    ...
    eh.request_click_with_fallback(
        ...
        noon_agent::EventSender::new(fb_tx, 0),
    );
    barrier(&host);
...
fn warm_click_survives_post_completion_cancel() {
    ...
    let (trigger, token) = noon_agent::CancelToken::new();
    ctx.cancel = token;
    exec_warm_tool(&reg, WARM_TOOL_NAME, &ctx).expect("tool output");
    let body = recv_live_buf(&rx, WARM_ID).expect("live buf published");
    trigger.cancel();
...
/// `noon.agent.call_tool` returns `(text, err)` and delivers live bufs and
...
noon.api.register_tool({{
    name = "driver",
    description = "dispatches children via noon.agent.call_tool",
    schema = {MINIMAL_SCHEMA},
    audiences = {{ "main" }},
    handler = function(input, ctx)
        local ann = "nil"
        local text, err = noon.agent.call_tool(ctx, "annotated_child", {{}}, {{
            on_annotation = function(a) ann = a end,
        }})
        local live_text = "none"
        local ann2 = "nil"
        local text2 = noon.agent.call_tool(ctx, "streaming_child", {{}}, {{
        ...
        local ann3 = "nil"
        local _, err3 = noon.agent.call_tool(ctx, "failing_child", {{}}, {{
...
fn jobstop_kills_running_job() {
    ...
    let src = format!(
        r#"noon.api.register_tool({{
            ...
            audiences = {{ "main" }},
            handler = function(input, ctx)
                local id = noon.fn.jobstart("sleep 60", {{
                    on_exit = function(job_id, code)
...
fn setup_happy_path() {
    ...
    let raw = host
        .send_run_init_lua(
            "noon.setup({ agent = { max_output_lines = 3000 } })".to_owned(),
...
#[test_case::test_case(
    r#"noon.setup({ agent = { compaction_buffer = 10000 } })"#,
    noon_config::CompactionBuffer::Tokens(10_000)
    ; "compaction_buffer_tokens"
)]
#[test_case::test_case(
    r#"noon.setup({ agent = { compaction_buffer = "15%" } })"#,
    noon_config::CompactionBuffer::Percent(15)
    ; "compaction_buffer_percent"
)]
fn setup_compaction_buffer(lua_src: &str, expected: noon_config::CompactionBuffer) {
    let reg = fresh_registry();
    let host = PluginHost::new(Arc::clone(&reg)).unwrap();
    let raw = host
...
#[test_case::test_case(
    "noon.setup({ ui = { splash_animaton = false } })",
    UNKNOWN_FIELD_ERR
    ; "unknown_field"
)]
#[test_case::test_case(
    r#"noon.setup({ agent = { max_output_lines = "not a number" } })"#,
    ""
    ; "wrong_type"
)]
...
(this plugin declares no options via noon.api.register_options)";
...
#[test_case::test_case(
    r#"noon.api.register_options({ timeout_secs = { default = 120 } })"#,
    OPTION_DESC_ERR
    ; "missing_desc"
)]
#[test_case::test_case(
    r#"noon.api.register_options({ bare = { desc = "no type or default" } })"#,
    OPTION_NO_TYPE_ERR
    ; "missing_type_and_default"
)]
#[test_case::test_case(
    r#"noon.api.register_options({ timeout_secs = { default = 120, mins = 5, desc = "T." } })"#,
    OPTION_SPEC_KEY_ERR
    ; "unknown_spec_key"
)]
#[test_case::test_case(
    r#"noon.api.register_options({ timeout_secs = { type = "string", default = 120, desc = "T." } })"#,
    OPTION_DEFAULT_TYPE_ERR
    ; "default_contradicts_type"
)]
#[test_case::test_case(
    r#"noon.api.register_options({ timeout_secs = { default = 1, min = 5, desc = "T." } })"#,
    OPTION_DEFAULT_MIN_ERR
    ; "default_below_min"
)]
#[test_case::test_case(
    r#"noon.api.register_options({ label = { type = "string", min = 1, desc = "L." } })"#,
    OPTION_MIN_ON_STRING_ERR
    ; "min_on_string"
)]
#[test_case::test_case(
    r#"noon.api.register_options({ enabled = { default = true, desc = "E." } })"#,
    OPTION_RESERVED_ERR
    ; "reserved_enabled"
)]
#[test_case::test_case(
    r#"
    noon.api.register_options({ a = { default = 1, desc = "A." } })
    noon.api.register_options({ b = { default = 2, desc = "B." } })
    "#,
    OPTION_TWICE_ERR
    ; "called_twice"
)]
...
fn builtin_opts_flow_from_setup_plugins() {
    ...
    let raw = host
        .send_run_init_lua(
            "noon.setup({ plugins = { grep = { search_result_limit = 42 } } })".to_owned(),
            "test_init.lua".to_owned(),
            None,
        )
        .unwrap()
...
fn setup_always_thinking_variants(lua_val: &str, expected: AlwaysThinking) {
    ...
    let raw = host
        .send_run_init_lua(
            format!("noon.setup({{ always_thinking = {lua_val} }})"),
            "test_init.lua".to_owned(),
            None,
        )
        .unwrap()
...
#[test_case::test_case(
    r#"noon.api.register_command({ name = "", handler = function() end })"#,
    "non-empty" ; "empty_name"
)]
#[test_case::test_case(
    r#"noon.api.register_command({ name = "/test", description = "no handler" })"#,
...
fn reload_replaces_commands() {
    ...
    host.load_source(
        "reload_cmd",
        r#"noon.api.register_command({ name = "/v1", handler = function() end })"#,
    )
    .unwrap();

    host.load_source(
        "reload_cmd",
        r#"noon.api.register_command({ name = "/v2", handler = function() end })"#,
    )
    .unwrap();
...
fn unload_clears_commands() {
    ...
    host.load_source(
        "cmd_only",
        r#"noon.api.register_command({ name = "/bye", handler = function() end })"#,
    )
    .unwrap();
...
fn restore_tool_async_ordering_and_delivery() {
    ...
    let bash_item = |id: &str| noon_lua::RestoreItem {
        tool: Arc::from("bash"),
        tool_use_id: id.to_owned(),
    ...
    let unknown_item = noon_lua::RestoreItem {
    ...
    let snapshots: Vec<noon_agent::Envelope> = rx.drain().collect();

    let tool_ids: Vec<&str> = snapshots
        .iter()
        .filter_map(|env| match &env.event {
            noon_agent::AgentEvent::ToolSnapshot { id, .. } => Some(id.as_str()),
            _ => None,
        })
        .collect();
...
fn exec_tool_with_perms(
    perms: noon_lua::PluginPermissions,
    src: &str,
    tool: &str,
    input: serde_json::Value,
) -> Result<String, String> {
...
#[test_case::test_case(
    "read_deny",
    r#"local ok, err = pcall(function() noon.fs.read("/etc/hostname") end)
                return tostring(err)"#,
    "fs_read"
    ; "fs_read_denied"
)]
#[test_case::test_case(
    "write_deny",
    r#"local ok, err = pcall(function() noon.fs.write("/tmp/test", "x") end)
                return tostring(err)"#,
    "fs_write"
    ; "fs_write_denied"
)]
#[test_case::test_case(
    "run_deny",
    r#"local ok, err = pcall(function() noon.fn.jobstart("echo hi") end)
                return tostring(err)"#,
    "run"
    ; "run_denied"
)]
fn denied_permission_blocks_api(tool_name: &str, handler_body: &str, expected_perm: &str) {
    ...
    let result = exec_tool_with_perms(
        noon_lua::PluginPermissions::denied(),
        &src,
        tool_name,
        serde_json::json!({}),
    )
...
fn user_plugin_with_fs_read_can_read_but_not_write() {
    let src = perm_tool_src(
        "rw_test",
        r#"local read_ok = pcall(function() noon.fs.read("/dev/null") end)
                local write_ok = pcall(function() noon.fs.write("/tmp/test", "x") end)
                return "read=" .. tostring(read_ok) .. ",write=" .. tostring(write_ok)"#,
    );
    let mut perms = noon_lua::PluginPermissions::denied();
    perms.set(noon_lua::Permission::FsRead, true);
...
fn builtin_plugin_has_all_permissions() {
    let src = perm_tool_src(
        "trusted_test",
        r#"local cwd_ok = pcall(function() noon.uv.cwd() end)
                local env_ok = pcall(function() noon.env.state_dir() end)
                return "cwd=" .. tostring(cwd_ok) .. ",env=" .. tostring(env_ok)"#,
    );
    let result = exec_tool_with_perms(
        noon_lua::PluginPermissions::trusted(),
        &src,
        "trusted_test",
        serde_json::json!({}),
    )
...
fn env_permission_guards_uv_and_env() {
    let src = perm_tool_src(
        "env_guard_test",
        r#"local cwd_ok = pcall(function() noon.uv.cwd() end)
                local home_ok = pcall(function() noon.uv.os_homedir() end)
                local env_ok = pcall(function() noon.env.state_dir() end)
                local exec_ok = pcall(function() noon.fn.executable("ls") end)
...
fn pure_functions_not_guarded() {
    let src = perm_tool_src(
        "pure_test",
        r#"local dirname_ok = pcall(function() noon.fs.dirname("/foo/bar") end)
                local basename_ok = pcall(function() noon.fs.basename("/foo/bar") end)
                local json_ok = pcall(function() noon.json.encode({a=1}) end)
...
noon.api.register_tool({{
    ...
    start = function(input, ctx)
        ...
        parts[1] = pair_err(noon.agent.call_tool(ctx, "st_plain", {{ code = "x" }})) and "call_tool_err"
            or "call_tool_ok"
...
fn recv_live_buf(
    rx: &flume::Receiver<noon_agent::Envelope>,
    id: &str,
) -> Option<Arc<noon_agent::SharedBuf>> {
    rx.drain().find_map(|env| match env.event {
        noon_agent::AgentEvent::LiveToolBuf { id: got, body } if got == id => Some(body),
        _ => None,
    })
}
...
fn interpreter_on_output_streams_lines() {
    ...
    let src = format!(
        r#"noon.api.register_tool({{
            ...
            handler = function(input, ctx)
                local lines = {{}}
                local result, err = noon.interpreter.run("print('a')\nprint('b')", {{
                    timeout = 10,
                    max_memory_mb = 50,
...
fn interp_tool_plugin(name: &str, python: &str, tools_lua: &str) -> String {
    format!(
        r#"noon.api.register_tool({{
            ...
            handler = function(input, ctx)
                local lines = {{}}
                local result, err = noon.interpreter.run("{python}", {{
                    timeout = 10,
                    max_memory_mb = 50,
...
fn call_tool_resolves_lua_tool_and_reports_unknown() {
    ...
    let src = format!(
        r#"noon.api.register_tool({{
            ...
            audiences = {{ "main" }},
            handler = function(input, ctx)
                local out, err = noon.agent.call_tool(ctx, "echo_", {{ msg = "hello" }})
                if err ~= nil then return "unexpected err: " .. err end
                local out2, err2 = noon.agent.call_tool(ctx, "no_such_tool_xyz", {{}})
...
fn session_opts_validation_rejects(opts: &str, expected: &str) {
    ...
    let src = format!(
        r#"noon.api.register_tool({{
            ...
            audiences = {{ "main" }},
            handler = function(input, ctx)
                local sess, err = noon.agent.session(ctx, {opts})
...
fn lua_tool_image_reply_maps_to_image_output() {
    ...
    let noon_agent::ToolOutput::Image { source, text } = out else {
        panic!("expected Image output, got {out:?}");
    };
    assert_eq!(source.media_type, noon_agent::ImageMediaType::Png);
...
fn call_tool_flattens_image_output_with_not_visible_note() {
    use noon_agent::tools::interpreter_bridge::IMAGE_NOT_VISIBLE_NOTE;
    ...
    let src = format!(
        r#"noon.api.register_tool({{
            ...
            audiences = {{ "main" }},
            handler = function(input, ctx)
                local out, err = noon.agent.call_tool(ctx, "img_probe", {{}})
                return err or out
            end
        }})"#
    );
...
fn view_image_tool_returns_image_output() {
    ...
    assert!(audience.contains(noon_agent::tools::ToolAudience::MAIN));
    assert!(!audience.contains(noon_agent::tools::ToolAudience::INTERPRETER));
...
fn view_image_small_gif_passes_through_unchanged() {
    ...
    assert_eq!(source.media_type, noon_agent::ImageMediaType::Gif);
    assert!(
...
fn interpreter_bridge_flattens_image_with_visibility_note() {
    ...
    let out = smol::block_on(noon_agent::tools::interpreter_bridge::dispatch(
        &ctx,
        "img_probe",
        &serde_json::json!({}),
    ))
    ...
    assert!(
        out.contains(noon_agent::tools::interpreter_bridge::IMAGE_NOT_VISIBLE_NOTE),
        "got: {out}"
    );
}

/// The sessions picker parks its command handler in a `win:recv` loop while a
/// `noon.async.run` task fetches the stored-session list. Queued async tasks
...
fn async_run_from_parked_command_handler_runs_promptly() {
    ...
    host.load_source(
        ...
        noon.api.register_command({
            ...
            handler = function()
                noon.async.run(function()
                    noon.ui.flash("task-ran")
                end)
                noon.async.await(1, function(_cb) end)
            end,
        })
        "#,
    )
    ...
    assert!(matches!(action, noon_lua::UiAction::Flash(msg) if msg == "task-ran"));
}
...
fn job_callbacks_fire_while_command_handler_parked() {
    ...
    host.load_source(
        ...
        noon.api.register_command({
            ...
            handler = function()
                noon.fn.jobstart("echo hi", {
                    on_stdout = function(_, line) noon.ui.flash("job:" .. line) end,
                })
                noon.async.await(1, function(_cb) end)
            end,
        })
    ...
    assert!(matches!(action, noon_lua::UiAction::Flash(msg) if msg == "job:hi"));
}
