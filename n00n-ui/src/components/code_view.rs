...
fn highlight_spans(hl: &mut n00n_highlight::Highlighter, text: &str) -> Vec<Span<'static>> {
...
fn render_code(
    mut hl: Option<n00n_highlight::Highlighter>,
    start_line: usize,
    code_lines: &[String],
    total_count: usize,
    max_lines: usize,
...
impl<'a> FileWalker<'a> {
    fn new(content: &'a str, syntax: &'static SyntaxReference, theme: Arc<Theme>) -> Self {
        Self {
            ...
            pos: 1,
            hl: n00n_highlight::Highlighter::new(syntax, theme),
        }
    }
...
fn render_grep_results(
    ...
    for entry in entries {
        ...
        let syntax = highlight.then(|| n00n_highlight::syntax_for_path(&entry.path));
...
pub fn render_tool_content(
    ...
    let (output_lines, output_trunc) = match output {
        Some(ToolOutput::ReadCode {
            ...
            highlight.then(|| n00n_highlight::Highlighter::for_path(path)),
            *start_line,
            code_lines,
            code_lines.len(),
            limits.output,
...
mod tests {
    ...
    fn fg_in_context(
        ...
        let mut hl = n00n_highlight::Highlighter::new(n00n_highlight::syntax_for_path(path), theme);
