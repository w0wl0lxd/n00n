...
impl InputBox {
    ...
    pub fn new(history: InputHistory, theme_engine: Arc<ThemeEngine>) -> Self {
        Self {
    ...
    pub fn view(
            ...
            self.buffer
                ...
                .flat_map(|(i, line)| {
                    ...
                    let shell_spans = if i == 0 {
                        shell_highlight_spans(line, &self.theme_engine)
                    } else {
                        None
                    };
                    wrap_line(
        ...
        if !self.pending_images.is_empty() {
            ...
            styled_lines.push(Line::from(Span::styled(
                label,
                self.theme_engine.current().input_placeholder,
            )));
        }
        ...
        let paragraph = Paragraph::new(text)
            .style(Style::new().fg(self.theme_engine.current().foreground))
...
fn shell_highlight_spans(line: &str, theme_engine: &ThemeEngine) -> Option<Vec<Span<'static>>> {
    ...
    let shell_style = theme_engine.current().shell_prefix;
...
mod tests {
    ...
    #[test]
    fn submit() {
        let mut input = InputBox::new(InputHistory::default(), crate::theme::test_engine());
    ...
    fn cursor_adds_extra_wrap_row_at_boundary() {
        ...
        let mut at_boundary = InputBox::new(InputHistory::default(), crate::theme::test_engine());
        type_text(&mut at_boundary, &"x".repeat(ew));

        let mut before_boundary =
            InputBox::new(InputHistory::default(), crate::theme::test_engine());
    ...
    fn render_input(
        ...
        render_input_with(
            ...
            height,
            false,
            Style::new().fg(theme_engine.current().mode_build),
        )
    }
    ...
    fn char_before_cursor_backslash(input: &str, expected: bool) {
        let mut input_box = InputBox::new(InputHistory::default(), crate::theme::test_engine());
    ...
    fn copy_text_includes_prefix() {
        let input = InputBox::new(InputHistory::default(), crate::theme::test_engine());
