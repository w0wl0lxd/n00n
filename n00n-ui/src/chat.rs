...
impl Chat {
    ...
    pub(crate) fn set_restore_channel(
        ...
        event_tx: Option<n00n_agent::EventSender>,
    ) {
        self.messages_panel
    ...
    pub fn handle_event(&mut self, event: AgentEvent, plan_path: Option<&Path>) -> ChatEventResult {
        match event {
            ...
            AgentEvent::FusionPhaseChanged { .. } | AgentEvent::SubagentHistory { .. } => {}
...
pub fn history_to_display<S: std::hash::BuildHasher>(
    ...
    let mut display = Vec::new();
    let mut restore_items: Vec<n00n_lua::RestoreItem> = Vec::new();
    for msg in messages {
        match msg.role {
            Role::User => {
                if msg
                ...
                for block in &msg.content {
                    match block {
                        ...
                        ContentBlock::ToolUse { id, name, input } => {
                            ...
                            if !rust_rendered {
                                restore_items.push(n00n_lua::RestoreItem {
...
pub(crate) fn restore_item_for(
    msg: &DisplayMessage,
    tool_output_lines: n00n_config::ToolOutputLines,
    theme_gen: u64,
) -> Option<n00n_lua::RestoreItem> {
...
mod tests {
    use super::*;
    use n00n_agent::{AgentEvent, ToolDoneEvent, ToolOutput, ToolStartEvent};
