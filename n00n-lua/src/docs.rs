...
//! methods with `#[n00n_lua_macro::lua_fn]` (Lua name from the fn ident, args
...
//! consts with `n00n_lua_macro::lua_table!` / `lua_class!`. The few functions
//! that cannot fit (raw `MultiValue`, Lua chunks, conditional registration)
//! and `n00n.setup` keep hand-written `FnDoc`s. `api_docs()` aggregates
//! everything for n00n-docgen, and the drift test below asserts docs match
//! the real `n00n` global.

pub struct ModuleDoc {
    /// Dotted path, e.g. "n00n.base64". Classes use a type name, e.g.
    /// "n00n.treesitter.Node".
    pub name: &'static str,
    pub kind: DocKind,
    pub desc: &'static str,
...
mod tests {
    ...
    fn resolve_table(n00n: &Table, path: &str) -> Table {
        let mut table = n00n.clone();
    ...
    fn docs_match_registered_api() {
        ...
        for (name, mut expected) in documented {
            let actual = table_keys(&resolve_table(&n00n, name));
            if name == "n00n" {
