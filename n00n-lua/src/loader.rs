...
static BUNDLED_PLUGINS: &[BundledPlugin] = &[
    ...
    bundled("github", include_dir!("$CARGO_MANIFEST_DIR/../plugins/github")),
    bundled(
        "view_image",
...
impl PluginHost {
    ...
    pub fn load_init_files(&self, cwd: &Path) -> Result<Option<RawConfig>, PluginError> {
        ...
        for global_dir in n00n_config::global_config_dirs() {
...
impl EventHandle {
    ...
    /// the whole batch has landed, n00nng it the batch barrier.
    #[doc(hidden)]
...
mod tests {
    ...
    fn live_event_handle_does_not_hang_after_begin_shutdown() {
        ...
        host.load_source(
            "hinted",
            r#"n00n.api.register_prompt_hint({ slot = "tool_usage", content = "live" })"#,
        )
        .unwrap();
    ...
    fn keybind_callback_runs_end_to_end() {
        ...
        host.load_source(
            "kb",
            r#"
            n00n.keymap.set("n", "<C-g>", function()
                n00n.api.register_command({
                    name = "/fired",
    ...
    fn disabled_host_skips_init_files(with_init: bool) {
        ...
        if with_init {
            fs::create_dir_all(dir.path().join(".n00n")).unwrap();
            fs::write(dir.path().join(".n00n/init.lua"), "error('should not run')").unwrap();
        }
        let host = PluginHost::disabled();
        let config = host
    ...
    fn multiple_plugins_sorted_by_plugin_name() {
        ...
        for plugin in ["zzz", "aaa"] {
            host.load_source(
                plugin,
                r#"
                n00n.api.register_prompt_hint({ slot = "tool_usage", content = "from_PLUGIN" })
                "#
    ...
    fn unload_clears_all_hints_from_plugin() {
        ...
        host.load_source(
            "multi",
            r#"
            n00n.api.register_prompt_hint({ slot = "tool_usage", prompt = "system", content = "usage" })
            n00n.api.register_prompt_hint({ slot = "conventions", prompt = "system", content = "conv" })
            "#,
        )
        .unwrap();
        let handle = host.event_handle().unwrap();
    ...
    fn invalid_hint_spec_is_rejected(spec: &str) {
        let host = PluginHost::new(Arc::new(ToolRegistry::new())).unwrap();
        let src = format!("n00n.api.register_prompt_hint({spec})");
    ...
    fn singleton_last_wins_across_plugins() {
        ...
        host.load_source(
            "aaa",
            r#"n00n.api.set_prompt({ slot = "identity", content = "AAA" })"#,
        )
        .unwrap();
        host.load_source(
            "zzz",
            r#"n00n.api.set_prompt({ slot = "identity", content = "ZZZ" })"#,
        )
        .unwrap();
        let slots = host.event_handle().unwrap().collect_prompt_slots();
    ...
    fn content_required() {
        let host = PluginHost::new(Arc::new(ToolRegistry::new())).unwrap();
        let r = host.load_source("bad", r#"n00n.api.set_prompt({ slot = "identity" })"#);
        assert!(r.is_err());
    ...
    fn set_prompt_invalid_prompt_rejected() {
        ...
        let r = host.load_source(
            "bad",
            r#"n00n.api.set_prompt({ slot = "identity", prompt = "nope", content = "x" })"#,
        );
        assert!(r.is_err());
    }
    ...
    fn set_prompt_and_register_prompt_hint_coexist() {
        ...
        host.load_source(
            "hint",
            r#"n00n.api.register_prompt_hint({ slot = "tool_usage", content = "HINT" })"#,
        )
        .unwrap();
        host.load_source(
            "setter",
            r#"n00n.api.set_prompt({ slot = "identity", content = "SET" })"#,
        )
        .unwrap();
        let slots = host.event_handle().unwrap().collect_prompt_slots();
        assert_eq!(
    ...
    fn set_prompt_rejects_aggregate_slot() {
        ...
        let r = host.load_source(
            "bad",
            r#"n00n.api.set_prompt({ slot = "tool_usage", content = "nope" })"#,
        );
        assert!(r.is_err());
    }
    ...
    fn set_prompt_rejects_incompatible_slot_prompt() {
        ...
        let r = host.load_source(
            "bad",
            r#"n00n.api.set_prompt({ slot = "identity", prompt = "research", content = "x" })"#,
        );
        assert!(r.is_err());
        assert!(r.unwrap_err().to_string().contains("not available"));
    }
    ...
    fn empty_prompt_table_is_rejected() {
        ...
        let r = host.load_source(
            "bad",
            r#"n00n.api.set_prompt({ slot = "identity", prompt = {}, content = "x" })"#,
        );
        assert!(r.is_err());
    ...
    fn content_must_not_be_empty() {
        ...
        let r = host.load_source(
            "bad",
            r#"n00n.api.set_prompt({ slot = "identity", content = "" })"#,
        );
        assert!(r.is_err());
