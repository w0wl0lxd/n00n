...
mod tests {
    ...
    #[test]
    fn ts_node_resolves_root_by_id() {
        let tree = parse_source();
        let root = LuaNode::new(tree.root_node(), Arc::clone(&tree));
        let resolved = root.ts_node().unwrap();
        assert_eq!(resolved.id(), tree.root_node().id());
        assert_eq!(resolved.kind(), tree.root_node().kind());
    }

    #[test]
    fn ts_node_resolves_nested_descendant_by_id() {
        let tree = parse_source();
        let fn_item = tree.root_node().child(0).unwrap();
        let lua_node = LuaNode::new(fn_item, Arc::clone(&tree));
        let resolved = lua_node.ts_node().unwrap();
        assert_eq!(resolved.id(), fn_item.id());
        assert_eq!(resolved.kind(), "function_item");
    }

    #[test]
    fn ts_node_errors_when_id_is_not_present_in_tree() {
        let tree = parse_source();
        let lua_node = LuaNode {
            tree: Arc::clone(&tree),
            id: usize::MAX,
        };
        let err = lua_node.ts_node().unwrap_err();
        assert!(err.to_string().contains("node not found in tree"));
    }

    #[test]
    fn wrap_preserves_tree_reference() {
        let tree = parse_source();
        let root = LuaNode::new(tree.root_node(), Arc::clone(&tree));
        let child_ts = tree.root_node().child(0).unwrap();
        let child = root.wrap(child_ts);
        assert!(Arc::ptr_eq(&root.tree, &child.tree));
        assert_eq!(child.ts_node().unwrap().id(), child_ts.id());
    }

    #[test]
    fn wrap_opt_returns_none_for_none() {
        let tree = parse_source();
        let root = LuaNode::new(tree.root_node(), Arc::clone(&tree));
        assert!(root.wrap_opt(None).is_none());
    }

    #[test]
    fn wrap_opt_returns_some_wrapped_node_for_some() {
        let tree = parse_source();
        let root = LuaNode::new(tree.root_node(), Arc::clone(&tree));
        let child_ts = tree.root_node().child(0).unwrap();
        let wrapped = root.wrap_opt(Some(child_ts)).unwrap();
        assert_eq!(wrapped.ts_node().unwrap().id(), child_ts.id());
    }
    ...
    fn clone_preserves_node_identity() {
        ...
        assert_eq!(
            root.ts_node().unwrap().id(),
            cloned.ts_node().unwrap().id()
        );
    }
}
