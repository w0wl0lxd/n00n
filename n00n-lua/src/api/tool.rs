...
use n00n_agent::prompt::{PromptId, Slot, SlotKind, ValidNames};
use n00n_agent::tools::Tool;
use n00n_agent::tools::registry::{RegisteredTool, ToolRegistry};
...
async fn recv_pre_exec_reply<T>(
    started_rx: Receiver<()>,
    reply_rx: Receiver<T>,
) -> Option<Result<T, flume::RecvError>> {
    recv_pre_exec_reply_with_timeouts(
        started_rx,
        reply_rx,
        PRE_EXEC_QUEUE_TIMEOUT,
        PRE_EXEC_CALLBACK_TIMEOUT,
    )
    .await
}

async fn recv_pre_exec_reply_with_timeouts<T>(
    started_rx: Receiver<()>,
    reply_rx: Receiver<T>,
    queue_timeout: Duration,
    callback_timeout: Duration,
) -> Option<Result<T, flume::RecvError>> {
    match recv_reply_with_timeout(started_rx, queue_timeout).await {
        Some(Ok(())) => recv_reply_with_timeout(reply_rx, callback_timeout).await,
    ...
    fn start_header(&self) -> HeaderFuture {
        ...
        HeaderFuture::Pending {
            ...
            fut: Box::pin(async move {
                ...
                match recv_pre_exec_reply(started_rx, reply_rx).await {
...
lua_table! {
    ...
    /// ```
    extend "n00n.api" => pub(crate) fn add_tool_fns(pending: PendingTools, plugin: Arc<str>, opts: PluginOpts), DOCS [
...
fn tool_entry_to_lua(lua: &Lua, entry: &RegisteredTool) -> LuaResult<Table> {
    ...
    for (flag, name) in n00n_agent::tools::registry::AUDIENCE_NAMES {
...
mod tests {
    ...
    fn describe_falls_back_to_static_description_when_runtime_unavailable() {
        use n00n_agent::tools::ToolFilter;
    ...
    #[test]
    fn pre_execution_callback_timeout_starts_after_dispatch() {
        smol::block_on(async {
            let (started_tx, started_rx) = flume::bounded(1);
            let (reply_tx, reply_rx) = flume::bounded(1);
    ...
    fn needs_compute_fallback_on_failure() {
        ...
        std::thread::spawn(move || {
            if let Ok(Request::ComputePermissionScopes { started, reply, .. }) = rx2.recv() {
                let _ = started.send(());
    ...
    fn needs_compute_returns_callback_result() {
        ...
        std::thread::spawn(move || {
            if let Ok(Request::ComputePermissionScopes { started, reply, .. }) = rx.recv() {
                let _ = started.send(());
