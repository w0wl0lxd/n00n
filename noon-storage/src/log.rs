...
const LOG_FILE_NAME: &str = "noon.log";
const LOCK_FILE_NAME: &str = "noon.log.lock";
...
fn file_path(dir: &Path, index: u32) -> PathBuf {
    if index == 0 {
        ...
        dir.join(format!("noon.{index}.log"))
    }
}
...
/// Logs used to live in the state dir. Move any leftover `noon.*.log` files
...
fn migrate_stale_logs(old_dir: &Path, new_dir: &Path) {
    ...
    for entry in entries.flatten() {
        ...
        if !name.starts_with("noon.") || !name.ends_with(".log") {
            continue;
        }
            ...
            for i in (0..last).rev() {
                ...
                if src.exists() {
                    ...
                    if let Err(e) = fs::rename(&src, &dst) {
                        eprintln!("noon: log rotate rename {src:?} -> {dst:?}: {e}");
                    }
                }
            }
        }
...
impl Write for RotatingFileWriter {
    fn write(&mut self, buf: &[u8]) -> io::Result<usize> {
        ...
        {
            eprintln!("noon: log rotation failed: {e}");
        }
...
mod tests {
    ...
    fn migrates_only_log_files_and_removes_lock() {
        ...
        assert!(new.join("noon.1.log").exists());
