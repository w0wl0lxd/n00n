...
/// The canonical unique id for anything in noon (sessions, and message
...
impl fmt::Display for NoonId {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.write_str(&bs58::encode(&self.0).into_string())
    }
}

impl FromStr for NoonId {
    type Err = NoonIdParseError;

    fn from_str(s: &str) -> Result<Self, Self::Err> {
        if s.is_empty() {
            return Err(NoonIdParseError::Empty);
        }
...
fn decode_base58(s: &str) -> Result<NoonId, NoonIdParseError> {
    let bytes = bs58::decode(s).into_vec().map_err(|e| match e {
        bs58::decode::Error::InvalidCharacter { character, index } => {
            NoonIdParseError::InvalidBase58(character, index)
        }
        bs58::decode::Error::NonAsciiCharacter { index } => {
            NoonIdParseError::InvalidBase58('\u{FFFD}', index)
        }
        _ => NoonIdParseError::InvalidBase58Length,
    })?;
    if bytes.len() != UUID_BYTES {
        return Err(NoonIdParseError::InvalidByteLen(bytes.len()));
    }
...
impl Serialize for NoonId {
    fn serialize<S: Serializer>(&self, ser: S) -> Result<S::Ok, S::Error> {
        ser.collect_str(self)
    }
}

impl<'de> Deserialize<'de> for NoonId {
...
/// unchanged) so wire echo and client correlation hold. The parsed [`NoonId`]
...
impl SessionRef {
    ...
    pub fn id(&self) -> NoonId {
        self.id
    }
}

impl From<NoonId> for SessionRef {
    fn from(id: NoonId) -> Self {
        Self::from_id(id)
    }
}
...
mod tests {
    ...
    fn roundtrip_base58() {
        ...
        assert_eq!(s.parse::<NoonId>().unwrap(), id);
    }
    ...
    fn roundtrips_leading_zero_bytes(hex: &str) {
        let id: NoonId = hex.parse().unwrap();
        assert_eq!(id.to_string().parse::<NoonId>().unwrap(), id);
    }

    #[test_case(SAMPLE_HEX)]
    #[test_case("019650874c717f008000000000000000")]
    fn parses_legacy_and_canonical(s: &str) {
        let expected = NoonId(Uuid::parse_str(SAMPLE_HEX).unwrap().into_bytes());
        assert_eq!(parse(s), expected);
    }

    #[test_case("" => matches Err(NoonIdParseError::Empty))]
    #[test_case("O" => matches Err(NoonIdParseError::InvalidBase58('O', 0)))]
    #[test_case("2j87v4grC" => matches Err(NoonIdParseError::InvalidByteLen(_)))]
    fn rejects_bad(s: &str) -> Result<NoonId, NoonIdParseError> {
        s.parse()
    }
    ...
    fn serde_keyed_base58() {
        ...
        let back: NoonId = serde_json::from_str(&s).unwrap();
        assert_eq!(back, id);
    }
