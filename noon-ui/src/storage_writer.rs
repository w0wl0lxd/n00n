...
use noon_storage::sessions::{SESSIONS_DIR, SessionError, SessionLog};
use tracing::warn;

use crate::AppSession;

type Pending = Arc<Mutex<HashMap<NoonId, Box<AppSession>>>>;

type DeleteCallback = Box<dyn FnOnce(Result<(), SessionError>) + Send>;

enum Op {
    Flush,
    Delete { id: NoonId, done: DeleteCallback },
}
...
impl StorageWriter {
    pub fn new(dir: StateDir) -> Self {
        ...
        std::thread::Builder::new()
            ...
            .spawn(move || {
                let mut logs: HashMap<NoonId, SessionLog> = HashMap::new();
    ...
    pub fn delete(&self, id: NoonId, done: impl FnOnce(Result<(), SessionError>) + Send + 'static) {
        let op = Op::Delete {
            id,
            done: Box::new(done),
        };
...
fn lock(pending: &Pending) -> std::sync::MutexGuard<'_, HashMap<NoonId, Box<AppSession>>> {
    pending.lock().unwrap_or_else(|e| e.into_inner())
}

fn writer_gone() -> SessionError {
    noon_storage::StorageError::Io(io::Error::other("storage writer unavailable")).into()
}

fn flush(pending: &Pending, logs: &mut HashMap<NoonId, SessionLog>, dir: &StateDir) {
...
fn write_session(
    sessions_dir: &Path,
    logs: &mut HashMap<NoonId, SessionLog>,
    session: &AppSession,
) {
