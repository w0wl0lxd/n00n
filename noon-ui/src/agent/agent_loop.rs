...
use noon_agent::agent;
use noon_agent::mcp::McpHandle;
use noon_agent::mcp::config::McpServerStatus;
...
use noon_providers::{AgentError, Message, Model, TokenUsage};
...
impl AgentLoop {
    ...
    async fn do_agent_run(
            ...
            for pm in messages {
                ...
                let msg = match pm.role {
                    PromptRole::Assistant => Message {
                        role: noon_providers::Role::Assistant,
                        content: vec![noon_providers::ContentBlock::Text { text }],
                        ..Default::default()
                    },
        ...
        let prompt_slots = match self.lua_handle.as_ref() {
            ...
            None => noon_agent::prompt::ResolvedSlots::default(),
        };
        ...
        .with_interrupt_source(Arc::clone(&self.queue) as Arc<dyn noon_agent::InterruptSource>)
        .with_cancel(cancel)
    ...
    fn publish_btw_system(&self, prompt_slots: &noon_agent::prompt::ResolvedSlots) {
        let slot = self.model_slot.load();
        let system = agent::build_system_prompt(
            &self.vars,
...
fn spawn_oauth_for_needs_auth(handle: &McpHandle) {
    ...
    for info in snapshot.infos.iter() {
        ...
        smol::spawn(async move {
            let storage = match noon_storage::StateDir::resolve() {
                Ok(s) => s,
                Err(e) => {
            ...
            if let Err(e) = noon_agent::mcp::oauth::authenticate(
                &server_name,
                &server_url,
                www_auth.as_deref(),
                &storage,
                noon_agent::mcp::oauth::Interaction::Background,
            )
            .await
            {
