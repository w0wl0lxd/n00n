...
use noon_providers::ModelTier;
use noon_providers::dynamic;
use noon_providers::model_registry;
...
fn parse_model_entry(spec: &str) -> Option<ModelEntry> {
    ...
    let provider_display = if let Ok(kind) = provider_str.parse::<ProviderKind>() {
        ...
        let config = noon_config::providers::ProvidersConfig::load();
        config.get(provider_str)?;
        noon_config::providers::resolve_display_name(provider_str, config.get(provider_str))
    };
    ...
    drop(map);
    let tier = override_label.unwrap_or_else(|| match noon_providers::Model::from_spec(spec) {
