...
fn highlight_spans(hl: &mut noon_highlight::Highlighter, text: &str) -> Vec<Span<'static>> {
...
fn render_code(
    mut hl: Option<noon_highlight::Highlighter>,
    start_line: usize,
    code_lines: &[String],
    total_count: usize,
    max_lines: usize,
    ...
    match syntax {
        ...
        None => {
            ...
            spans.push(Span::styled(
                noon_highlight::normalize_text(&full),
                base.patch(theme::current().code_block),
            ));
        }
    }
...
fn render_grep_results(
    ...
    for entry in entries {
        ...
        let syntax = highlight.then(|| noon_highlight::syntax_for_path(&entry.path));
        ...
        for (gi, group) in entry.groups.iter().enumerate() {
            ...
            for line in &group.lines {
                ...
                let text_spans = if let Some(syn) = syntax {
                    highlight_spans(
                        &mut noon_highlight::Highlighter::for_syntax(syn),
                        &line.text,
                    )
...
pub(crate) fn render_instructions(
    ...
    for (i, block) in blocks.iter().enumerate() {
        ...
        let hl = highlight.then(|| noon_highlight::Highlighter::for_path(&block.path));
...
pub fn render_tool_content(
    ...
    if let Some((language, code)) = input.map(|i| match i {
        ...
        let code_lines: Vec<String> = code
            ...
            .collect();
        let total = code_lines.len();
        let hl = highlight.then(|| noon_highlight::Highlighter::for_token(language));
    ...
    let (output_lines, output_trunc) = match output {
        Some(ToolOutput::ReadCode {
            ...
            highlight.then(|| noon_highlight::Highlighter::for_path(path)),
            *start_line,
            code_lines,
            code_lines.len(),
            limits.output,
...
mod tests {
    ...
    fn render_code_line_count(input_lines: usize, total: usize, expected: usize) {
        ...
        let (result, _) = render_code(
            Some(noon_highlight::Highlighter::for_path("test.rs")),
            1,
            &code_lines,
            total,
            READ_MAX_LINES,
