...
use noon_agent::{McpPromptInfo, McpSnapshotReader};
use noon_lua::{LuaCommandInfo, LuaCommandReader};
...
mod tests {
    use super::*;
    use noon_agent::{McpPromptArg, McpSnapshot};
    ...
    fn sample_custom() -> Arc<[CustomCommand]> {
        Arc::from([
            CustomCommand {
                ...
                scope: noon_agent::command::CommandScope::Project,
                accepts_args: true,
            },
            CustomCommand {
                name: "fix".into(),
                ...
                scope: noon_agent::command::CommandScope::User,
                accepts_args: false,
            },
        ])
    }
    ...
    fn lua_commands_update_on_generation_change() {
        let (writer, reader) = noon_lua::test_support::lua_command_writer_pair();
