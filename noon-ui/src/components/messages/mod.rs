...
use crate::theme;
use noon_config::{ToolOutputLines, UiConfig};
...
impl MessagesPanel {
    ...
    fn lua_restore_item(&self, tool_id: &str) -> Option<noon_lua::RestoreItem> {
        let msg = self
            .messages
            .iter()
            .rfind(|m| matches!(&m.role, DisplayRole::Tool(t) if t.id == tool_id))?;
