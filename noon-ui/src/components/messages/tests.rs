...
use noon_agent::tools::{BASH_TOOL_NAME, GREP_TOOL_NAME, WRITE_TOOL_NAME};
use noon_agent::{
...
fn extract_partial_column_selection() {
    ...
    let world_start = NOON_PREFIX_LEN + "Hello ".len() as u16;
...
fn extract_mixed_fully_enclosed_and_partial() {
    ...
    let sel = make_sel(area, (0, 0), (seg1_start as u32, NOON_PREFIX_LEN + 6));
    let text = panel.extract_selection_text(&sel, area);
...
fn extract_partial_col_symmetric(msgs: &[&str], expect_start: &str, expect_end: &str) {
    ...
    let total: u16 = panel.segment_heights().iter().sum();
    let area = Rect::new(0, 0, 80, 24);
    let down = make_sel(area, (0, NOON_PREFIX_LEN), ((total - 1) as u32, 79));
    let up = make_sel(area, ((total - 1) as u32, 79), (0, NOON_PREFIX_LEN));
...
fn second_register_live_buf_replaces_first() {
    let preview = Arc::new(noon_agent::SharedBuf::new());
    preview.set_lines(vec![snap_line("preview")]);
    let handler = Arc::new(noon_agent::SharedBuf::new());
...
fn handle_click_on_watched_tool_sends_click_with_fallback(is_error: bool) {
    let (eh, probe) = noon_lua::test_support::probed_event_handle();
...
fn rebake_walk_requests_without_stamping_displayed_generation() {
    ...
    panel.set_restore_channel(
        Some(noon_lua::EventHandle::disconnected_for_test()),
...
fn test_event_sender() -> noon_agent::EventSender {
    let (tx, _rx) = flume::unbounded();
