...
use noon_agent::permissions::PermissionManager;
use noon_agent::{
...
use noon_config::{PermissionsConfig, UiConfig};
use noon_lua::{HintReader, KeymapReader, LuaCommandReader};
use noon_providers::{ContentBlock, Effort, Role, TokenUsage};
use noon_storage::sessions::{StoredMode, StoredThinking};
...
fn reload_leaves_empty_session_unpersisted_on_disk() {
    ...
    let sessions_dir = tmp.path().join(noon_storage::sessions::SESSIONS_DIR);
...
fn cd_command_behavior() {
    ...
    let resolved = noon_storage::paths::canonicalize_clean(Path::new("/tmp"));
...
fn rewind_to_middle_truncates_and_populates_input() {
    ...
    let expected_ctx = noon_agent::agent::estimate_message_tokens(&app.state.session.messages);
...
fn override_shadows_builtin_ctrl_when_no_overlay_open() {
    let entry = noon_lua::KeymapEntry {
        key: kb::HELP.code,
    ...
    let reader = noon_lua::test_support::keymap_reader_with(vec![entry]);
    let mut app = test_app();
    let (handle, _probe) = noon_lua::test_support::probed_event_handle();
...
fn dead_host_override_falls_back_to_builtin() {
    ...
    let reader = noon_lua::test_support::keymap_reader_with(vec![entry]);
    let mut app = test_app();
    app.lua_event_handle = Some(noon_lua::EventHandle::disconnected_for_test());
    app.keymap_reader = reader;
    assert!(!app.help_modal.is_open());
...
fn set_opus_model(app: &mut App) {
    app.state.model = noon_providers::Model::from_spec("anthropic/claude-opus-4-8").unwrap();
}
...
fn fast_flashes_error_on_ineligible_model(spec: &str) {
    let mut app = test_app();
    app.state.model = noon_providers::Model::from_spec(spec).unwrap();
...
fn update_model_to_ineligible_resets_fast() {
    ...
    let sonnet = noon_providers::Model::from_spec("anthropic/claude-sonnet-4-5").unwrap();
...
fn ctrl_c_denies_permission_prompt() {
    ...
    app.permission_prompt.open(
        "id".into(),
        noon_config::ToolKey::native("bash"),
        vec!["execute".into()],
        None,
    );
...
fn open_split_window(app: &mut App, dir: noon_lua::Split) {
    let buf = Arc::new(noon_agent::SharedBuf::new());
    let config = noon_lua::FloatConfig {
        width: noon_lua::Dimension::Abs(SPLIT_EXTENT),
        height: noon_lua::Dimension::Abs(SPLIT_EXTENT),
        border: noon_lua::Border::None,
        split: dir,
        ..noon_lua::FloatConfig::default()
    };
    let (event_tx, _event_rx) = flume::bounded::<noon_lua::WinEvent>(8);
    let (_cmd_tx, cmd_rx) = flume::bounded::<noon_lua::WinCommand>(8);
...
#[test_case(noon_lua::Split::Above ; "above")]
#[test_case(noon_lua::Split::Left ; "left")]
#[test_case(noon_lua::Split::Right ; "right")]
fn non_below_split_reserves_band_and_keeps_status_full_width(dir: noon_lua::Split) {
    let mut app = test_app();
...
fn permission_prompt_takes_bottom_precedence_over_below_split() {
    let mut app = test_app();
    open_split_window(&mut app, noon_lua::Split::Below);
    open_split_window(&mut app, noon_lua::Split::Left);
    open_split_window(&mut app, noon_lua::Split::Above);
    app.permission_prompt.open(
        "perm-1".into(),
        noon_config::ToolKey::native("bash"),
        vec!["ls".into()],
    ...
    assert!(
        splits.rect(noon_lua::Split::Below).is_none(),
        "below split must yield the bottom area to an open permission prompt",
    );
    assert!(
        splits.rect(noon_lua::Split::Left).is_some(),
        "the prompt must leave a left split untouched",
    );
    assert!(
        splits.rect(noon_lua::Split::Above).is_some(),
