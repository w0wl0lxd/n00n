...
use noon_providers::{Model, TokenUsage};
use noon_storage::id::NoonId;
use noon_storage::sessions::StoredSubagent;
...
pub(crate) fn session_has_content(session: &AppSession) -> bool {
    !session.messages.is_empty()
        ...
        || session.meta.mode != Some(noon_storage::sessions::StoredMode::Build)
}
...
impl App {
    ...
    fn fire_restore_items(&self, items: Vec<noon_lua::RestoreItem>) {
    ...
    pub(super) fn rewind_to(&mut self, entry: RewindEntry) -> Vec<Action> {
        ...
        self.state.context_size =
            noon_agent::agent::estimate_message_tokens(&self.state.session.messages);
    ...
    pub(crate) fn load_session(&mut self, session_id: NoonId) -> Vec<Action> {
