...
use noon_config::Effect;
use noon_providers::{Message, Model, ThinkingConfig, TokenUsage};
use noon_storage::StateDir;
use noon_storage::sessions::{StoredEffect, StoredMode, StoredRule};
...
impl SessionState {
    ...
    pub fn sync_session(
        ...
        self.session.updated_at = noon_storage::now_epoch();
...
pub(crate) fn rules_to_stored(rules: &[noon_config::PermissionRule]) -> Vec<StoredRule> {
    rules
        .iter()
        .map(|r| {
...
fn migrate_stored_tool_key(s: &str) -> Option<noon_config::ToolKey> {
        ...
        && let Some((server, tool)) = rest.split_once("__")
    {
        ...
        return noon_config::ToolKey::parse(&new_form)
            .map_err(
                |e| tracing::warn!(key = s, error = %e, "malformed stored tool key — skipping"),
            )
            .ok();
    }
    match noon_config::ToolKey::parse(s) {
        Ok(key) => Some(key),
        Err(e) => {
...
pub(crate) fn stored_to_rules(stored: &[StoredRule]) -> Vec<noon_config::PermissionRule> {
    stored
        .iter()
        .filter_map(|r| {
            ...
            Some(noon_config::PermissionRule {
                tool,
                scope: r.scope.clone(),
                effect,
            })
