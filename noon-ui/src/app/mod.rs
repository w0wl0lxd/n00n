...
use noon_lua::{EventHandle, HintReader, KeymapReader, LuaCommandReader};
use noon_providers::{Message, Model, ThinkingConfig};
use noon_storage::StateDir;
use noon_storage::input_history::InputHistory;
...
const AUTH_EXPIRED_MSG: &str =
    "Token expired. Run `noon auth login` in another terminal, then press Enter to retry.";
...
pub struct App {
    ...
    pub(crate) restore_event_tx: Option<noon_agent::EventSender>,
...
impl App {
    ...
    pub fn new(
        ...
        custom_commands: Arc<[noon_agent::command::CustomCommand]>,
    ) -> Self {
        ...
        app.model_picker
            .set_recents(noon_storage::model::read_recents(&app.storage));
        app
    }
    ...
    pub(crate) fn record_recent_model(&mut self, spec: &str) {
        let recents = noon_storage::model::push_recent(&self.storage, spec);
    ...
    fn execute_mcp_prompt(&mut self, name: &str, args: &str) -> Vec<Action> {
        ...
        let prompt_ref = noon_agent::McpPromptRef {
    ...
    fn cmd_cd(&mut self, args: &str) -> Vec<Action> {
        let path = if args.is_empty() {
            noon_storage::paths::home().unwrap_or_default()
        } else {
            match args.strip_prefix('~') {
                Some(rest) => {
                    let home = noon_storage::paths::home().unwrap_or_default();
                    if rest.is_empty() {
                        home
                    } else {
