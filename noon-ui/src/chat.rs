...
use noon_agent::tools::{ToolInvocation, ToolRegistry, WRITE_TOOL_NAME};
use noon_agent::{AgentEvent, BufferSnapshot, ToolDoneEvent, ToolOutput, ToolStartEvent};
use noon_config::{ToolKey, ToolOutputLines, UiConfig};
use noon_providers::{ContentBlock, Message, Role, TokenUsage};
use ratatui::Frame;
...
impl Chat {
    ...
    pub(crate) fn set_restore_channel(
        &mut self,
        event_handle: Option<noon_lua::EventHandle>,
        event_tx: Option<noon_agent::EventSender>,
    ) {
        self.messages_panel
...
) -> (Vec<DisplayMessage>, Vec<noon_lua::RestoreItem>) {
    let results = build_tool_results_map(messages);
    let mut display = Vec::new();
    let mut restore_items: Vec<noon_lua::RestoreItem> = Vec::new();
    for msg in messages {
        match msg.role {
            Role::User => {
                ...
                for block in &msg.content {
                    match block {
                        ...
                        ContentBlock::ToolUse { id, name, input } => {
                            ...
                            if !rust_rendered {
                                restore_items.push(noon_lua::RestoreItem {
...
pub(crate) fn restore_item_for(
    msg: &DisplayMessage,
    tool_output_lines: noon_config::ToolOutputLines,
    theme_gen: u64,
) -> Option<noon_lua::RestoreItem> {
...
mod tests {
    use super::*;
    use noon_agent::{AgentEvent, ToolDoneEvent, ToolOutput, ToolStartEvent};
