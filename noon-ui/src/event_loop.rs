...
use noon_agent::command::CustomCommand;
use noon_agent::permissions::PermissionManager;
use noon_agent::{AgentConfig, CancelToken, McpCommand, McpConfigErrors, McpHandle, mcp};
use noon_config::UiConfig;
use noon_lua::{
    EventHandle, HintReader, KeymapReader, LuaCommandReader, SessionReply, SessionRequest, UiAction,
};
use noon_providers::Timeouts;
use noon_providers::provider::{Provider, fetch_all_models, from_model};
use noon_providers::{Message, Model};
use noon_storage::StateDir;
use noon_storage::StorageError;
use noon_storage::id::{NoonId, NoonIdParseError, SessionRef};
use noon_storage::sessions::{SessionError, normalize_title};
use serde_json::json;
use tracing::warn;
...
fn parse_session_id(id: &str) -> Result<NoonId, String> {
    id.parse().map_err(|e: NoonIdParseError| e.to_string())
}
...
enum Wake {
    ...
    InputGone,
    Ui(UiAction),
    Agent(usize, Box<noon_agent::Envelope>),
...
fn merge_batch(
    available: &Arc<ArcSwapOption<Vec<String>>>,
    batch: noon_providers::provider::ModelBatch,
    warn_tx: &flume::Sender<String>,
) {
...
impl<'t> EventLoop<'t> {
    pub(crate) fn new(
        ...
        let provider: Arc<dyn Provider> = if needs_login {
            Arc::from(noon_providers::provider::from_model_fallback(
                &mut model, timeouts,
            ))
        } else {
    ...
    fn handle_agent(&mut self, idx: usize, envelope: Box<noon_agent::Envelope>) {
            ...
            SessionRequest::SetTitle { id, title } => {
                ...
                let reply = (|| {
                    ...
                    if let Some(i) = self.position(id) {
                        ...
                        session.title = title;
                        session.updated_at = noon_storage::now_epoch();
    ...
    fn position(&self, id: NoonId) -> Option<usize> {
    ...
    fn focus_session(&mut self, id: NoonId) -> Result<(), String> {
        if let Some(i) = self.position(id) {
            self.set_focus(i);
            return Ok(());
        }
    ...
    fn handle_action(&mut self, idx: usize, action: Action) {
        match action {
            ...
            Action::AssignTier(spec, tier) => {
                noon_providers::model_registry::set_and_persist(spec, tier, &self.ctx.storage);
            }
            Action::UnassignTier(spec, tier) => {
                noon_providers::model_registry::unset_and_persist(&spec, tier, &self.ctx.storage);
            }
            Action::Compact => {
                let rt = &mut self.sessions[idx];
                let run_id = rt.app.run_id;
    ...
    fn refresh_provider(&mut self, slug: String) {
        ...
        if model.provider.to_string() == slug {
            if let Ok(provider) =
                noon_providers::provider::from_model(&mut model, self.ctx.timeouts)
            {
        ...
        } else if let Some(builtin) = noon_config::providers::builtin_provider(&slug) {
