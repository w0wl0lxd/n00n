...
pub use noon_highlight::TAB_SPACES;

pub(crate) fn warmup() {
    refresh_syntax_theme();
    noon_highlight::warmup();
}

pub(crate) fn is_ready() -> bool {
    noon_highlight::is_ready()
}

pub(crate) fn refresh_syntax_theme() {
    let theme = theme::current();
    noon_highlight::set_theme(theme.syntax.clone());
    noon_highlight::set_ui_colors(
        [
...
pub fn highlight_line(hl: &mut noon_highlight::Highlighter, text: &str) -> Vec<Span<'static>> {
...
pub fn fallback_span(text: &str) -> Span<'static> {
    Span::styled(
        noon_highlight::normalize_text(text),
...
pub fn highlight_ansi(lang: &str, code: &str) -> String {
    ...
    noon_highlight::highlight_ansi(lang, code, bg)
}
