...
case("largest_entries_sorts_descending_and_caps_at_top_n", function()
  local tmpdir = mktmpdir()
  n00n.fs.write(n00n.fs.joinpath(tmpdir, "small.md"), "12")
  n00n.fs.write(n00n.fs.joinpath(tmpdir, "big.md"), "1234567890")
  n00n.fs.write(n00n.fs.joinpath(tmpdir, "medium.md"), "12345")

  local top2 = h.largest_entries(tmpdir, 2)
  eq(#top2, 2, "should cap at top_n")
  eq(top2[1][1], "big.md", "largest entry first")
  eq(top2[2][1], "medium.md", "second largest second")

  local top_all = h.largest_entries(tmpdir, 10)
  eq(#top_all, 3, "top_n beyond count returns all entries")
  rmtree(tmpdir)
end)

case("largest_entries_empty_dir", function()
  local tmpdir = mktmpdir()
  eq(#h.largest_entries(tmpdir, 3), 0, "empty dir has no entries")
  rmtree(tmpdir)
end)

case("format_largest_entries_reports_names_and_sizes", function()
  local tmpdir = mktmpdir()
  n00n.fs.write(n00n.fs.joinpath(tmpdir, "notes.md"), "1234567890")
  local formatted = h.format_largest_entries(tmpdir, 3)
  assert(formatted:find("notes.md"), "should name the file")
  assert(formatted:find("10 bytes"), "should report its size")
  rmtree(tmpdir)
end)

case("format_largest_entries_empty_dir_returns_empty_string", function()
  local tmpdir = mktmpdir()
  eq(h.format_largest_entries(tmpdir, 3), "", "no hint for an empty directory")
  rmtree(tmpdir)
end)
