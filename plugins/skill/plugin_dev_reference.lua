...
**Returns:** (`maki.async.Semaphore`) A new semaphore.
...
**Returns:** (`maki.async.Permit`) A permit handle. Call `:release()` when done, or let it be garbage collected.
...
**Returns:** (`maki.image.Image?`, `string?`) Decoded image, or `(nil, err)` on failure.
...
**Returns:** (`maki.image.Image`) A new image handle (or the same one if no resize was needed).
...
**Returns:** (`maki.json.SchemaValidator?`, `string?`) Validator, or nil plus an error.
...
- `{node}` (`Node`) The node whose text you want.
...
- `{dest}` (`Node`) Potential ancestor node.
- `{source}` (`Node`) Node to check ancestry for.
...
- `{node}` (`Node`) Node to test against.
...
- `{node}` (`Node`) Root node to search within.
