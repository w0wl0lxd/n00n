...
local function render_skeleton(view, text, meta)
  local hl_entries = {}
  for line_nr, line in ipairs(n00n.split(text:gsub("\n+$", ""), "\n")) do
...
n00n.api.register_tool({
  ...
  schema = {
    ...
    required = { "path" },
    properties = {
      path = { type = "string", required = true, description = "File path to index (absolute, relative, or ~/)" },
    },
  },
  ...
  handler = function(input, ctx)
    ...
    local source, err = n00n.fs.read(path)
    if not source then
