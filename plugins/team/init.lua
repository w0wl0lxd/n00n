...
local function run_autonomous(ctx, goal, input, steps, relay_k, logger, resume_state)
  ...
  return results, total_cost, failures, nil
end
...
local function handler(input, ctx)
  ...
  if not steps then
    steps, perr = run_supervisor(ctx, goal, input)
  end
  if perr then
    if logger then
  ...
  if input.mode == "swarm" then
    if gate.fan_out then
      ...
      return finish_run(ctx, input, results, out.cost or 0, out.rounds or 0, "rounds", slug, 0, logger)
    end

    -- β gate says don't fan out: single strong-agent pass, log the reason.
    local results, total_cost, failures, pause = run_single_pass(ctx, goal, input, steps, relay_k, logger, resume_state)
    if pause then
      if logger then
        logger.log(
          "human_escalation",
    ...
    return finish_run(ctx, input, results, total_cost, #results, "steps", slug, failures, logger)
  end

  local results, total_cost, failures, pause = run_autonomous(ctx, goal, input, steps, relay_k, logger, resume_state)
  if pause then
    if logger then
      logger.log(
        "human_escalation",
...
finish_run = function(ctx, input, results, total_cost, completed, unit, slug, failures, logger)
