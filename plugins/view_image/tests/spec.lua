-- Exercises the noon.image / noon.base64 primitives view_image is built
-- from. The full tool path is covered by the view_image tests in
-- noon-lua/tests/plugin_host.rs.
...
case("base64_roundtrip", function()
  local bytes = noon.base64.decode(TINY_PNG_B64)
  assert(noon.base64.encode(bytes) == TINY_PNG_B64, "roundtrip mismatch")
end)

case("probe_reports_png_dimensions", function()
  local bytes = noon.base64.decode(TINY_PNG_B64)
  local info, err = noon.image.probe(bytes)
  assert(err == nil, err)
...
case("probe_rejects_non_image", function()
  local info, err = noon.image.probe("definitely not an image")
...
case("decode_rejects_truncated_image", function()
  local bytes = noon.base64.decode(TINY_PNG_B64)
  local img, err = noon.image.decode(bytes:sub(1, math.floor(#bytes / 2)))
...
case("decode_resize_encode_pipeline", function()
  local bytes = noon.base64.decode(TINY_PNG_B64)
  local img, err = noon.image.decode(bytes)
  assert(err == nil, err)
  ...
  local info = noon.image.probe(png)
  assert(info.format == "png")

  local jpeg = img:encode("jpeg")
  local jinfo = noon.image.probe(jpeg)
