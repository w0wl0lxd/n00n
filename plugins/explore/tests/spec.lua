...
case("explicit_command_selects_relations", function()
  eq(router.normalize_intent({ query = "foo", command = "callers" }), "relations")
end)
...
case("what_calls_routes_to_callers", function()
  eq(router.parse_relations_command({ query = "what calls restore_item" }), "callers")
  eq(router.extract_symbol("what calls restore_item", "callers"), "restore_item")
end)

case("what_does_call_routes_to_callees", function()
  eq(router.parse_relations_command({ query = "what does restore_item call" }), "callees")
...
case("cache_key_is_stable", function()
  local key_a = router.cache_key("codegraph", { command = "callers", projectPath = "." })
  local key_b = router.cache_key("codegraph", { projectPath = ".", command = "callers" })
  eq(key_a, key_b)
end)
...
case("search_intent_routes_to_search_text", function()
  ...
  eq(backend, "search_text")
  eq(input.command, "search")
  eq(input.query, "agent loop")
end)

case("search_backend_input_is_the_degraded_route", function()
  local input = router.search_backend_input({ query = "agent loop", project = "/tmp/p" })
  eq(input.command, "search")
  eq(input.query, "agent loop")
  eq(input.repo, "/tmp/p")
  eq(input.mode, "bm25")
  local _, routed = router.build_backend_input({ query = "agent loop", intent = "search" }, "search")
  eq(routed.command, input.command)
...
case("relations_intent_falls_back_to_query", function()
  local backend, input = router.build_backend_input({ query = "restore_item", intent = "relations" }, "relations")
  eq(backend, "map_codegraph")
  eq(input.command, "query")
  eq(input.query, "restore_item")
end)
