Noon is an AI coding agent (like Claude Code and opencode), that is built bottom up to optimize costs and number of tokens used, while not sacrificing performance too much.
...
- noon-ui: Uses ratatui for an interactive UI (elm like architecture)
- noon-providers: Integration with LLM providers via APIs (e.g. Anthropic, Z.AI)
- noon-agent: An async agent loop that runs on smol
- noon-interpreter: code_execution tool implementation using pydantic/monty (a minimal python sandbox)
- noon-storage: Persistent state across runs (e.g. sessions, auth)
- noon-config: User config
- noon-lua: Lua plugin system (API mirrored from neovim for plugin compatibility), built-in plugins in ./plugins dir
- noon-acp: ACP ndjson stdio server
...
Homepage: ./site/. User docs: ./site/docs/. Most pages are generated by `noon-docgen/src/gen_*.rs`, run `just gen-docs` to regenerate.
