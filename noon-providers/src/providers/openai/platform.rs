...
impl OpenAi {
    ...
    async fn refresh_oauth(&self) -> Result<(), AgentError> {
        ...
        let resolved = smol::unblock(move || {
            let tokens =
                noon_storage::auth::load_tokens(&storage, auth::PROVIDER).ok_or_else(|| {
                    AgentError::Api {
                        status: 401,
            ...
            match auth::refresh_tokens(&tokens) {
                Ok(fresh) => {
                    noon_storage::auth::save_tokens(&storage, auth::PROVIDER, &fresh)?;
                    Ok(auth::build_oauth_resolved(&fresh))
                }
                Err(e) => {
                    warn!(error = %e, "OpenAI OAuth refresh failed, clearing stale tokens");
                    let _ = noon_storage::auth::delete_tokens(&storage, auth::PROVIDER);
                    Err(e)
                }
            }
        })
    ...
    fn codex_auth(&self) -> Result<ResolvedAuth, AgentError> {
        ...
        if let Some(storage) = self.storage.as_ref()
            && let Some(tokens) = noon_storage::auth::load_tokens(storage, auth::PROVIDER)
        {
