...
use noon_storage::auth::{OAuthTokens, delete_tokens, load_tokens, now_millis, save_tokens};
...
pub fn resolve(dir: &StateDir) -> Result<ResolvedAuth, AgentError> {
    ...
    if let Some(creds) = noon_storage::auth::load_provider_credentials(dir, PROVIDER) {
    ...
    Err(AgentError::Config {
        message: "not authenticated, run `noon auth login openai` or set OPENAI_API_KEY".into(),
    })
}
