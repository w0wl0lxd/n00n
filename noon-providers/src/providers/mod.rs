...
impl KeyPool {
    ...
    pub fn resolve(slug: &str, env_var: &str) -> Result<Self, AgentError> {
        ...
        Err(AgentError::Config {
            message: format!(
                "{env_var} not set and no saved credentials for '{slug}' — run `noon auth login {slug}`"
            ),
        })
    }

    fn key_from_file(slug: &str) -> Option<String> {
        let dir = noon_storage::StateDir::resolve().ok()?;
        noon_storage::auth::load_provider_credentials(&dir, slug).map(|c| c.api_key)
    }
...
mod tests {
    ...
    #[test]
    fn resolve_from_env() {
        let env_var = format!("NOON_TEST_KEY_{}", fastrand::u32(..));
    ...
    fn resolve_env_supports_comma_separated() {
        let env_var = format!("NOON_TEST_MULTI_{}", fastrand::u32(..));
    ...
    fn resolve_returns_error_when_nothing_found() {
        ...
        let env_var = format!("NOON_TEST_KEY_NONE_{}", fastrand::u32(..));
