...
impl AgentError {
    ...
    pub fn user_message(&self) -> String {
        match self {
            ...
            Self::Api { status: 401, .. } => {
                "authentication failed, run `noon auth login` or check your API key".into()
            }
...
impl From<noon_storage::StorageError> for AgentError {
    fn from(e: noon_storage::StorageError) -> Self {
        match e {
            noon_storage::StorageError::Io(io) => Self::Io(io),
            noon_storage::StorageError::Json(j) => Self::Json(j),
            other => Self::Api {
                status: 0,
...
mod tests {
    ...
    #[test_case(401, "authentication failed, run `noon auth login` or check your API key" ; "user_msg_401")]
