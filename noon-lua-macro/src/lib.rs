//! Proc macros keeping `noon-lua` API registration and docs in one place.
//!
...
/// `lua_table! { /// desc … "noon.x" => pub(crate) fn create_x_table(perms: &PluginPermissions), DOCS [a(perms), b, manual c] }`
///
...
/// `lua_class! { /// desc … "noon.ui.Buf" => BufHandle, DOCS [a, b, manual c] fields f, extra g }`
///
