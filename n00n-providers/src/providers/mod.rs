...
impl KeyPool {
    pub fn from_env(env_var: &str) -> Result<Self, AgentError> {
        let raw = std::env::var(env_var).map_err(|error| match error {
            std::env::VarError::NotPresent => AgentError::Config {
    ...
    pub fn resolve(slug: &str, env_var: &str) -> Result<Self, AgentError> {
        ...
        match env_error {
            AgentError::Config { .. } => Err(AgentError::Config {
                message: format!(
    ...
    fn key_from_file(slug: &str) -> Option<String> {
        let dir = n00n_storage::StateDir::resolve().ok()?;
...
mod tests {
    ...
    fn resolve_returns_error_when_nothing_found() {
        ...
        let env_var = format!("N00N_TEST_KEY_NONE_{}", fastrand::u32(..));
