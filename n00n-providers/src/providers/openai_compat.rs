...
pub async fn parse_sse(
    ...
    for (idx, acc) in tool_accumulators.into_iter().enumerate() {
        let input: Value = match serde_json::from_str(&acc.arguments) {
            Ok(v) => {
                debug!(tool = %acc.name, json = %redact_json_arg(&acc.arguments), "tool input JSON");
                v
            }
            Err(e) => {
                warn!(error = %e, tool = %acc.name, json = %redact_json_arg(&acc.arguments), "malformed tool JSON, falling back to {{}}");
                Value::Object(serde_json::Map::default())
            }
        };
        let id = if acc.id.is_empty() {
            warn!(raw_name = %acc.name, raw_args = %redact_json_arg(&acc.arguments), "provider sent empty tool_use id; substituting placeholder");
            format!("n00n_unnamed_{idx}")
        } else {
            acc.id
        };
        let name = if acc.name.is_empty() {
            warn!(%id, raw_args = %redact_json_arg(&acc.arguments), "provider sent empty tool_use name; substituting placeholder");
