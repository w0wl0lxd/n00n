...
use noon_config_macro::ConfigSection;
use noon_storage::paths;
use noon_storage::sessions::{StoredThinking, ThinkingParseError};
use serde::{Deserialize, Serialize};
use serde_json::{Map as JsonMap, Value as JsonValue};
use thiserror::Error;
use tracing::warn;

const PROJECT_DIR: &str = ".noon";
...
pub struct PluginFileConfig {
    ...
    /// and validates its own via `noon.api.register_options`.
    #[serde(flatten)]
...
pub struct PluginsConfig {
    ...
    /// own via `noon.api.register_options` at load time.
...
fn migrate_permissions_file(path: &Path) -> Option<String> {
    ...
    if let Err(e) = noon_storage::atomic_write(path, new_content.as_bytes()) {
...
fn append_global_permission(
    ...
    noon_storage::atomic_write(&path, doc.to_string().as_bytes())
...
fn append_project_permission(
    ...
    let mut doc: toml_edit::DocumentMut = content
        .parse()
        .map_err(|e| format!("failed to parse .noon/{PERMISSIONS_FILE}: {e}"))?;

    insert_permission_entry(&mut doc, tool, scope, effect)?;

    if let Some(parent) = path.parent() {
        std::fs::create_dir_all(parent).map_err(|e| format!("cannot create .noon dir: {e}"))?;
    }
    noon_storage::atomic_write(&path, doc.to_string().as_bytes())
        .map_err(|e| format!("cannot write .noon/{PERMISSIONS_FILE}: {e}"))?;
    Ok(())
}
...
mod tests {
    use super::*;
    use noon_storage::sessions::Effort;
    use std::fs;
    ...
    fn write_global_permissions(dir: &Path, content: &str) {
        let perms_dir = dir.join(".config/noon");
    ...
    fn permissions_merge_global_and_project() {
        ...
        let noon_dir = dir.path().join(".noon");
        fs::create_dir_all(&noon_dir).unwrap();
        fs::write(
            noon_dir.join("permissions.toml"),
            "[read]\nallow = true\n\
             [write]\ndeny = [\"/etc/*\"]\n",
        )
        .unwrap();
    ...
    fn project_default_allow_ignored() {
        ...
        let noon_dir = dir.path().join(".noon");
        fs::create_dir_all(&noon_dir).unwrap();
        fs::write(noon_dir.join("permissions.toml"), "default = \"allow\"\n").unwrap();
    ...
    fn project_default_deny_allowed() {
        ...
        let noon_dir = dir.path().join(".noon");
        fs::create_dir_all(&noon_dir).unwrap();
        fs::write(noon_dir.join("permissions.toml"), "default = \"deny\"\n").unwrap();
    ...
    fn env_file_precedence() {
        const GLOBAL_ONLY: &str = "TEST_NOON_GLOBAL_ONLY";
        const PROJECT_SHADOWS: &str = "TEST_NOON_PROJECT_SHADOWS";
        const PROCESS_WINS: &str = "TEST_NOON_PROCESS_WINS";
    ...
    fn migrate_mcp_old_flat_keys() {
        ...
        let global = global_config_dir(dir.path());
        fs::create_dir_all(&global).unwrap();
        // Old noon format used quoted TOML keys for mcp:server__tool
        fs::write(
