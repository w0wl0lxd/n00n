...
/// Coarse capability classification used by noon-providers to dispatch tiered
/// requests. Mirrors `noon_providers::ModelTier` shape but lives here so the
/// config layer can validate inputs without depending on noon-providers.
...
impl ProvidersConfig {
    ...
    /// every provider and starting noon with an empty registry.
    pub fn load() -> Self {
